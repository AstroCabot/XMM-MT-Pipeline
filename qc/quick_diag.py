...
# Sibling module — works both as ``python -m qc.quick_diag`` and as a
# subprocess call ``python qc/quick_diag.py`` (the runner adds qc/ to
# sys.path before invoking).
try:
...
def main():
    ...
    qpu.savefig(fig, outdir / "lightcurve.png")
    ...
    for idx in range(nrows * ncols):
        ...
        ax.set_title(bin_labels[idx], fontsize=8, color=color)
        ax.set_xlabel("East offset (arcsec)", fontsize=7)
        ax.set_ylabel("North offset (arcsec)", fontsize=7)
        ax.tick_params(labelsize=6)

    qpu.savefig(fig, outdir / "mosaic.png")
    print(f"  -> {outdir / 'mosaic.png'}")

    # ---- Full / quiet comparison ----
    ...
    fig, axes = qpu.make_figure(
        1, 3, figscale=5.0,
        suptitle=f"Comet-frame comparison \u2014 PI [{args.pimin}:{args.pimax}]",
    )

    panel_data = [
        (full_img, f"All data ({n_total:,} events)"),
        (
            quiet_img,
    ...
    for (img, title), ax in zip(panel_data, axes[0]):
    ...
    qpu.savefig(fig, outdir / "comparison.png")
