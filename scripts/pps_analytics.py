...
def extract_attitude(attfile):
    with fits.open(attfile) as hdul:
        ...
        return {'time': data['TIME'].copy(), 'ra': data['AHFRA'].copy(), 'dec': data['AHFDEC'].copy(), 'pa': data['AHFPA'].copy()}
...
def extract_source_list(srcfile):
    with fits.open(srcfile) as hdul:
        ...
        result = {'columns': cols}
        for c in ('RA', 'DEC', 'SRC_NUM', 'PN_CTS', 'M1_CTS', 'M2_CTS', 'EP_EXTENT', 'EP_EXTENT_ERR', 'EP_DET_ML', 'EP_8_DET_ML'):
            if c in cols:
...
def extract_image(imgfile):
    with fits.open(imgfile) as hdul:
        ...
        data = hdul[0].data
        wcs_info = {}
        for k in ('CRVAL1', 'CRVAL2', 'CDELT1', 'CDELT2', 'CRPIX1', 'CRPIX2', 'NAXIS1', 'NAXIS2'):
            if k in hdr:
                wcs_info[k] = hdr[k]
...
def plot_gti_timeline(evtfiles, fbkfiles, tref, outpath):
    ...
    for fbk in fbkfiles:
        ...
        ax.plot(t_rel[good], fr['rate'][good], lw=0.5, alpha=0.8, color=colors_inst.get(inst, 'gray'), label=inst)
    ...
    for evtf in evtfiles:
        ...
        for gti_name, (starts, stops) in gtis.items():
            for s, e in zip(starts, stops):
                ax.barh(y, (e - s) / 1000, left=(s - tref) / 1000, height=0.6, color=colors_inst.get(inst, 'gray'), alpha=0.4, edgecolor='none')
            break
...
def plot_flare_rates(fbkfiles, tref, outpath):
    ...
    for ax, fbk in zip(axes, scheduled):
        ...
        if 'gti_start' in fr:
            for s, e in zip(fr['gti_start'], fr['gti_stop']):
                ax.axvspan((s - tref) / 1000, (e - tref) / 1000, alpha=0.1, color='green')
            ax.text(0.99, 0.95, f"Clean: {fr['gti_clean_ks']:.1f} ks", transform=ax.transAxes, ha='right', va='top', fontsize=8, bbox=dict(facecolor='white', alpha=0.7))
    ...
    fig.suptitle('PPS flare background rates with clean GTI shading (green)', fontsize=11)
    fig.tight_layout()
    fig.savefig(str(outpath), dpi=150)
    plt.close(fig)
...
def plot_source_map(srcdata, imgfile, outpath):
    ...
    x_grid = wcs['CRVAL1'] + wcs['CDELT1'] * (np.arange(nx) - wcs.get('CRPIX1', nx // 2) + 1)
    y_grid = wcs['CRVAL2'] + wcs['CDELT2'] * (np.arange(ny) - wcs.get('CRPIX2', ny // 2) + 1)
    ...
    if 'RA' in srcdata and 'DEC' in srcdata:
        ax.scatter(srcdata['RA'], srcdata['DEC'], s=60, facecolors='none', edgecolors='cyan', lw=1, label=f"{len(srcdata['RA'])} sources")
        for i in range(len(srcdata['RA'])):
            label = str(srcdata.get('SRC_NUM', np.arange(len(srcdata['RA'])))[i])
            ax.annotate(label, (srcdata['RA'][i], srcdata['DEC'][i]), color='cyan', fontsize=6, xytext=(3, 3), textcoords='offset points')
...
def plot_image_bands(pps_dir, inst_prefix, outpath):
    band_map = {'1000': '0.2-0.5 keV', '2000': '0.5-2.0 keV', '3000': '2.0-4.5 keV', '4000': '4.5-7.5 keV', '5000': '7.5-12 keV', '8000': '0.2-12 keV'}
    images = []
    labels = []
    wcs_list = []
...
def write_summary(outpath, pps_dir, mt_dir, evtfiles, fbkfiles, attdata, srcdata):
    ...
    for evtf in evtfiles:
        ...
        for gname, (starts, stops) in gtis.items():
            ...
            lines.append(f'    {gname}: {len(starts)} intervals, {total_ks:.1f} ks clean')
        lines.append('')
    ...
    for fbk in fbkfiles:
        ...
        if len(finite_rates) > 0:
            lines.append(f'    Rate range: {finite_rates.min():.2f} - {finite_rates.max():.2f} ct/s')
    ...
    if attdata:
        ...
        lines.append(f"""  RA range:  {attdata['ra'].min():.4f} - {attdata['ra'].max():.4f} deg (span {(attdata['ra'].max() - attdata['ra'].min()) * 3600:.1f}")""")
        lines.append(f"""  DEC range: {attdata['dec'].min():.4f} - {attdata['dec'].max():.4f} deg (span {(attdata['dec'].max() - attdata['dec'].min()) * 3600:.1f}")""")
        lines.append(f"  PA range:  {attdata['pa'].min():.4f} - {attdata['pa'].max():.4f} deg")
        lines.append('')
...
def main():
    ...
    parser.add_argument('--mt', type=Path, default=None, help='Path to MT_PPS directory')
    parser.add_argument('--workdir', type=Path, default=None, help='Pipeline WORKDIR for comparison (optional)')
    parser.add_argument('-o', '--outdir', type=Path, required=True, help='Output directory (must NOT be inside PPS or MT_PPS)')
    ...
    for data_dir in [pps_dir, mt_dir]:
        if data_dir and (outdir == data_dir or data_dir in outdir.parents):
            print(f'ERROR: output directory {outdir} is inside data directory {data_dir}', file=sys.stderr)
            print('Refusing to write into original data. Use a different -o path.', file=sys.stderr)
            return 1
    ...
    print()
    evtfiles = glob_ftz(primary_dir, '*PIEVLI*.FTZ') + glob_ftz(primary_dir, '*MIEVLI*.FTZ')
    ...
    for prefix in ('PNS', 'M1S', 'M2S'):
        ...
        if imgs:
            plot_image_bands(primary_dir, prefix, outdir / f'pps_image_bands_{prefix}.png')
            print(f"  -> {outdir / f'pps_image_bands_{prefix}.png'}")
    summary = write_summary(outdir / 'pps_summary.txt', pps_dir, mt_dir, evtfiles, fbkfiles, attdata, srcdata)
