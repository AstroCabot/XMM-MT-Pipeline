#!/usr/bin/env python3
"""Generate analytics plots and summary from PPS / MT_PPS data products.

Produces GTI timelines, flare-rate plots, attitude timeseries,
source maps, energy-band images, and a text summary.
"""
import argparse
import sys
...
def write_summary(outpath, pps_dir, mt_dir, evtfiles, fbkfiles, attdata, srcdata):
    ...
    if attdata:
        ...
        lines.append(
            f"""  RA range:  {attdata['ra'].min():.4f} - {attdata['ra'].max():.4f} deg (span {(attdata['ra'].max() - attdata['ra'].min()) * 3600:.1f}")"""
        )
        lines.append(
            f"""  DEC range: {attdata['dec'].min():.4f} - {attdata['dec'].max():.4f} deg (span {(attdata['dec'].max() - attdata['dec'].min()) * 3600:.1f}")"""
        )
        lines.append(
