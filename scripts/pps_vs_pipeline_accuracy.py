#!/usr/bin/env python3
"""Side-by-side comparison of PPS products vs pipeline outputs.

Compares housekeeping GTIs, flare filtering, event counts, source
detection, attitude, flare-rate statistics, and pipeline stage status.
"""
import sys
...
def main():
    workdir = Path(
        "/mnt/c/Users/cabot/OneDrive/Documents/Research/Cambridge/X3I/reduction_v4/output"
    )
    mt_dir = Path(
        "/mnt/c/Users/cabot/OneDrive/Documents/Research/Data/xmm_3iatlas_updated/MT_PPS"
    )
    outdir = Path(
        "/mnt/c/Users/cabot/OneDrive/Documents/Research/Cambridge/X3I/reduction_v4/xmm_comet_pipeline_refactor_release/pps_analytics_output"
    )
    ...
    for sf in pps_src_files:
        if "EPX" in sf.name:
            with fits.open(str(sf)) as hdul:
                ...
                ml = d["EP_DET_ML"]
                lines.append(
                    f"PPS:      {len(d)} EPIC sources (ML {ml.min():.1f} - {ml.max():.1f})"
                )
    ...
    if pipe_src.exists():
        with fits.open(str(pipe_src)) as hdul:
            ...
            ml = d["DET_ML"]
            lines.append(
                f"Pipeline: {len(d)} curated sources (ML {ml.min():.1f} - {ml.max():.1f}), band PI 1000-12000"
            )
    lines.append("")
    ...
    if att_files:
        with fits.open(str(att_files[0])) as hdul:
            ...
            lines.append(
                f'PPS attitude: {dt:.1f} ks, RA span {(ra.max() - ra.min()) * 3600:.1f}", DEC span {(dec.max() - dec.min()) * 3600:.1f}"'
            )
    ...
    for inst in ["PN", "MOS1", "MOS2"]:
        with fits.open(str(mt_dir / pps_fbk[inst])) as hdul:
            ...
            r = r[good]
            lines.append(
                f"PPS {inst}: median={np.median(r):.2f}, mean={np.mean(r):.2f}, max={r.max():.1f} ct/s, P90={np.percentile(r, 90):.1f}, P95={np.percentile(r, 95):.1f}, P99={np.percentile(r, 99):.1f}"
            )
    lines.append("")
    lines.append("=" * 72)
