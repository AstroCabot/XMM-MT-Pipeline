...
def write_output(lcs: list[LightCurve], out_fits: str, out_csv: str | None) -> None:
    ...
    if any((lc.fracexp is not None for lc in lcs)):
        ...
        total_frac = None
    cols = [fits.Column(name='TIME', format='D', unit='s', array=base.time.astype(np.float64)), fits.Column(name='RATE', format='E', unit='count / s', array=total_rate.astype(np.float32)), fits.Column(name='ERROR', format='E', unit='count / s', array=total_error.astype(np.float32))]
