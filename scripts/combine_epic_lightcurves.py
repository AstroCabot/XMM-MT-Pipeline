#!/usr/bin/env python3
"""Combine per-instrument corrected light curves into an EPIC total.

Reads individual corrected (epiclccorr output) light curves, bins them
onto a common time grid, and sums rates and errors in quadrature.
"""
...
class LightCurve:
    ...
    def __init__(self, path: str):
        ...
        with fits.open(path) as hdul:
            ...
            self.backv = (
                np.asarray(tab["BACKV"], dtype=float)
                if "BACKV" in hdu.columns.names
                else None
            )
            self.backe = (
                np.asarray(tab["BACKE"], dtype=float)
                if "BACKE" in hdu.columns.names
                else None
            )
            self.fracexp = (
                np.asarray(tab["FRACEXP"], dtype=float)
                if "FRACEXP" in hdu.columns.names
                else None
            )
            self.extname = hdu.name if hdu.name else "RATE"
            self.timedel = float(
                hdu.header.get("TIMEDEL", self.primary.get("TIMEDEL", np.nan))
            )


def assert_same_grid(lcs: list[LightCurve], tol: float = 1e-06) -> None:
    base = lcs[0]
    for other in lcs[1:]:
        if len(other.time) != len(base.time):
            raise RuntimeError(
                f"Light curves have different lengths: {base.path} vs {other.path}"
            )
        if not np.allclose(other.time, base.time, atol=tol, rtol=0.0):
            raise RuntimeError(
                f"Light curves are not aligned in TIME: {base.path} vs {other.path}"
            )
        if np.isfinite(base.timedel) and np.isfinite(other.timedel):
            if abs(base.timedel - other.timedel) > tol:
                raise RuntimeError(
                    f"Light curves have different TIMEDEL: {base.path} vs {other.path}"
                )
...
def write_output(lcs: list[LightCurve], out_fits: str, out_csv: str | None) -> None:
    ...
    total_rate = np.where(
        np.any(good, axis=0), np.nansum(np.where(good, rates, 0.0), axis=0), np.nan
    )
    total_error = np.where(
        np.any(good, axis=0),
        np.sqrt(np.nansum(np.where(good, errors, 0.0) ** 2, axis=0)),
        np.nan,
    )
    ninstr = np.sum(good, axis=0).astype(np.int16)
    if any((lc.backv is not None for lc in lcs)):
        backv_stack = np.vstack(
            [
                np.asarray(
                    (
                        lc.backv
                        if lc.backv is not None
                        else np.full_like(base.time, np.nan)
                    ),
                    dtype=float,
                )
                for lc in lcs
        ...
        total_backv = np.where(
            np.any(np.isfinite(backv_stack), axis=0),
            np.nansum(backv_stack, axis=0),
            np.nan,
        )
    else:
        total_backv = None
    if any((lc.backe is not None for lc in lcs)):
        backe_stack = np.vstack(
            [
                np.asarray(
                    (
                        lc.backe
                        if lc.backe is not None
                        else np.full_like(base.time, np.nan)
                    ),
                    dtype=float,
                )
        ...
        total_backe = np.where(
            np.any(np.isfinite(backe_stack), axis=0),
            np.sqrt(np.nansum(backe_stack**2, axis=0)),
            np.nan,
        )
    else:
        total_backe = None
    if any((lc.fracexp is not None for lc in lcs)):
        frac_stack = np.vstack(
            [
                np.asarray(
                    (
                        lc.fracexp
        ...
        total_frac = np.where(
            np.any(np.isfinite(frac_stack), axis=0),
            np.nanmean(frac_stack, axis=0),
            np.nan,
        )
    else:
        total_frac = None
    cols = [
        fits.Column(
            name="TIME", format="D", unit="s", array=base.time.astype(np.float64)
        ),
        fits.Column(
            name="RATE",
            format="E",
            unit="count / s",
            array=total_rate.astype(np.float32),
        ),
        fits.Column(
            name="ERROR",
            format="E",
            unit="count / s",
            array=total_error.astype(np.float32),
        ),
    ]
    if total_backv is not None:
        cols.append(
            fits.Column(
                ...
                format="E",
                unit="count / s",
                array=total_backv.astype(np.float32),
            )
        )
    if total_backe is not None:
        cols.append(
            fits.Column(
                ...
                format="E",
                unit="count / s",
                array=total_backe.astype(np.float32),
            )
        )
    if total_frac is not None:
        cols.append(
            fits.Column(name="FRACEXP", format="E", array=total_frac.astype(np.float32))
        )
    ...
    rate_hdu.header["HISTORY"] = (
        "RATE is the sum of epiclccorr-corrected instrument rates"
    )
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        nargs="+",
        required=True,
        help="Corrected light curves from epiclccorr",
    )
