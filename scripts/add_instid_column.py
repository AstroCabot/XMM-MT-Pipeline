#!/usr/bin/env python3
"""Add a constant INSTID column to the EVENTS table of an XMM event file."""
            ...
            if isinstance(hdu, fits.BinTableHDU) and hdu.name == "EVENTS":
                ...
                if "INSTID" in names:
                    ...
                    for col in hdu.columns:
                        if col.name == "INSTID":
                            cols.append(fits.Column(name="INSTID", format="I", array=arr))
                        else:
                            cols.append(col)
                    new_evt = fits.BinTableHDU.from_columns(cols, name="EVENTS")
                else:
                    new_evt = fits.BinTableHDU.from_columns(hdu.columns + fits.ColDefs([fits.Column(name="INSTID", format="I", array=arr)]), name="EVENTS")
