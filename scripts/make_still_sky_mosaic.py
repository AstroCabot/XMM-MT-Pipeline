...
from trail_mask_tools import read_srclist, read_track
...
def _event_radec(hdu):
    ...
    if 'RA' in cols_upper and 'DEC' in cols_upper:
        ...
        return (np.asarray(hdu.data[ra_col], dtype=float), np.asarray(hdu.data[dec_col], dtype=float))
    h = hdu.header
    xcol = ycol = None
...
def wrap_delta_ra_deg(ra_deg: np.ndarray, center_ra_deg: float) -> np.ndarray:
    return (np.asarray(ra_deg, dtype=float) - float(center_ra_deg) + 180.0) % 360.0 - 180.0

def offsets_arcsec(ra_deg: np.ndarray, dec_deg: np.ndarray, center_ra_deg: float, center_dec_deg: float) -> tuple[np.ndarray, np.ndarray]:
...
def choose_center(events: list[str], track_path: str | None, center_ra: float | None, center_dec: float | None, pimin: int, pimax: int) -> tuple[float, float]:
...
def selected_offsets(path: str, center_ra: float, center_dec: float, pimin: int, pimax: int) -> tuple[np.ndarray, np.ndarray, int]:
    with fits.open(path, memmap=True) as hdul:
        if 'EVENTS' not in hdul:
        ...
        pi = np.asarray(data['PI'], dtype=float)
        ra, dec = _event_radec(evt)
    m = np.isfinite(pi) & np.isfinite(ra) & np.isfinite(dec) & (pi >= pimin) & (pi <= pimax)
    if not np.any(m):
        return (np.asarray([], dtype=float), np.asarray([], dtype=float), 0)
...
def main() -> int:
    ...
    center_ra, center_dec = choose_center(events=args.events, track_path=args.track, center_ra=args.center_ra, center_dec=args.center_dec, pimin=args.pimin, pimax=args.pimax)
    ...
    for path in args.events:
        dx, dy, nsel = selected_offsets(path, center_ra, center_dec, args.pimin, args.pimax)
        nevents_total += nsel
        if dx.size:
        ...
        all_stats.append({'path': str(Path(path).resolve()), 'selected_events': int(nsel)})
    if args.track:
    ...
    for path in args.events:
        dx, dy, _nsel = selected_offsets(path, center_ra, center_dec, args.pimin, args.pimax)
        if dx.size == 0:
            continue
    ...
    if args.out_json:
        payload = {'label': str(args.label), 'pimin': int(args.pimin), 'pimax': int(args.pimax), 'center_ra_deg': float(center_ra), 'center_dec_deg': float(center_dec), 'x_min_arcsec': float(xmin), 'x_max_arcsec': float(xmax), 'y_min_arcsec': float(ymin), 'y_max_arcsec': float(ymax), 'bin_arcsec': float(bin_arcsec), 'selected_events_total': int(nevents_total), 'inputs': all_stats}
