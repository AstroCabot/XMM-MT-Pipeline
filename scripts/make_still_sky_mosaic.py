#!/usr/bin/env python3
"""Build a still-sky mosaic for source-detection QC.

Histograms cleaned events into a sky image centred on the mean
comet position, with the track and detected sources overlaid.
"""
...
def wrap_delta_ra_deg(ra_deg: np.ndarray, center_ra_deg: float) -> np.ndarray:
    return (
        np.asarray(ra_deg, dtype=float) - float(center_ra_deg) + 180.0
    ) % 360.0 - 180.0
