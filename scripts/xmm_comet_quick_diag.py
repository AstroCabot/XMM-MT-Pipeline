...
def main():
    ...
    parser.add_argument('--nbins', type=int, default=10, help='Number of time bins (default: 10)')
    parser.add_argument('--pimin', type=int, default=200, help='PI min (default: 200)')
    parser.add_argument('--pimax', type=int, default=2000, help='PI max (default: 2000)')
    parser.add_argument('--bin-arcsec', type=float, default=8.0, help='Image pixel size in arcsec (default: 8)')
    parser.add_argument('--inst', nargs='+', default=['PN', 'M1', 'M2'], help='Instruments to use')
    parser.add_argument('--radius-arcsec', type=float, default=900.0, help='Image half-width in arcsec')
    parser.add_argument('--src-r-arcsec', type=float, default=500.0, help='Source aperture radius for overlay')
    parser.add_argument('--lc-binsize', type=float, default=1000.0, help='Light curve bin size in seconds')
    ...
    ax.bar(lc_t_rel, lc_rates, width=args.lc_binsize / 1000, color=colors, edgecolor='none')
    ax.axhline(median_rate, color='k', ls='--', lw=0.8, label=f'median = {median_rate:.1f}')
    ax.axhline(flare_thresh, color='red', ls=':', lw=0.8, label=f'3× median = {flare_thresh:.1f}')
    ...
    for idx in range(nrows * ncols):
        ...
        if idx >= args.nbins:
            ...
            continue
        img = images[idx]
        extent_arcsec = [-args.radius_arcsec, args.radius_arcsec, -args.radius_arcsec, args.radius_arcsec]
        ax.imshow(img, origin='lower', extent=extent_arcsec, cmap='magma', vmin=0, vmax=vmax, interpolation='nearest')
        circ = plt.Circle((0, 0), args.src_r_arcsec, ec='cyan', fc='none', lw=1, ls='--')
        ax.add_patch(circ)
    ...
    fig.suptitle(f'Quick comet-frame diagnostic — PI [{args.pimin}:{args.pimax}], {args.bin_arcsec}" pixels, {args.nbins} time bins', fontsize=10)
    fig.tight_layout()
    ...
    quiet_img = make_image(all_x[quiet_sel], all_y[quiet_sel], xmin, xmax, ymin, ymax, binsize_phys)
    n_quiet = quiet_sel.sum()
    n_total = len(all_t)
    print(f'  Quiet events: {n_quiet:,} / {n_total:,} ({100 * n_quiet / n_total:.0f}%)')
    fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(15, 5))
    extent_arcsec = [-args.radius_arcsec, args.radius_arcsec, -args.radius_arcsec, args.radius_arcsec]
    for ax, img, title in [(ax1, full_img, f'All data ({n_total:,} events)'), (ax2, quiet_img, f'Quiet only ({n_quiet:,} events, {100 * n_quiet / n_total:.0f}%)'), (ax3, full_img - quiet_img * (n_total / n_quiet) if n_quiet > 0 else full_img, 'Flare excess (all − scaled quiet)')]:
        v = np.percentile(img[img > 0], 99) if np.any(img > 0) else 1
        ax.imshow(img, origin='lower', extent=extent_arcsec, cmap='magma', vmin=0, vmax=v, interpolation='nearest')
        circ = plt.Circle((0, 0), args.src_r_arcsec, ec='cyan', fc='none', lw=1, ls='--')
        ax.add_patch(circ)
        ax.set_title(title, fontsize=9)
        ax.set_xlabel('East offset (arcsec)')
        ax.set_ylabel('North offset (arcsec)')
    fig.suptitle(f'Comet-frame comparison — PI [{args.pimin}:{args.pimax}]', fontsize=11)
    fig.tight_layout()
