...
def read_track(track_fits: str) -> TrackData:
    ...
    return TrackData(mjd=mjd, ra=ra, dec=dec, mjdref=mjdref, timesys=timesys, obs_t0=obs_t0, obs_t1=obs_t1, t_sec=t_sec)
...
def read_srclist(src_fits: str) -> SrcList:
    with fits.open(src_fits) as hdul:
        ...
        if data is None or len(data) == 0:
            return SrcList(srcid=np.asarray([], dtype=int), ra=np.asarray([], dtype=float), dec=np.asarray([], dtype=float))
...
def clip_track_to_time_range(track: TrackData, t_min_sec: float | None=None, t_max_sec: float | None=None) -> TrackData:
    ...
    if t_lo <= float(track.t_sec.min()) + 1e-09 and t_hi >= float(track.t_sec.max()) - 1e-09:
        return track
    interp = TrackInterpolator(track.t_sec, track.ra, track.dec)
    keep = (track.t_sec > t_lo) & (track.t_sec < t_hi)
    times = np.concatenate([np.asarray([t_lo], dtype=float), np.asarray(track.t_sec[keep], dtype=float), np.asarray([t_hi], dtype=float)])
    ...
    return TrackData(mjd=np.asarray(mjd_utc, dtype=float), ra=np.asarray(ra, dtype=float), dec=np.asarray(dec, dtype=float), mjdref=track.mjdref, timesys=track.timesys, obs_t0=t_lo, obs_t1=t_hi, t_sec=np.asarray(times, dtype=float))

def build_trail_mask(grid: dict[str, Any], track_fits: str, srclist_fits: str, mask_radius_arcsec: float, shape: tuple[int, int], t_min_sec: float | None=None, t_max_sec: float | None=None) -> np.ndarray:
...
def _filter_events(event_path: str, mask: np.ndarray, grid: dict[str, float]) -> tuple[fits.HDUList, int, int, int]:
    with fits.open(event_path) as hdul:
        ...
        in_bounds = np.isfinite(col) & np.isfinite(row) & (col_i >= 0) & (col_i < mask.shape[1]) & (row_i >= 0) & (row_i < mask.shape[0])
...
def cmd_make(args: argparse.Namespace) -> int:
    ...
    if args.mask_radius_arcsec <= 0.0 or src_rows == 0:
        ...
        mask = build_trail_mask(grid=grid, track_fits=args.track, srclist_fits=args.srclist, mask_radius_arcsec=float(args.mask_radius_arcsec), shape=(ny, nx), t_min_sec=args.track_tmin_sec, t_max_sec=args.track_tmax_sec).astype(np.int16)
    hdr = fits.Header()
    ...
    if args.out_json:
        payload = {"grid_json": str(Path(args.grid_json).resolve()), "track": str(Path(args.track).resolve()), "srclist": str(Path(args.srclist).resolve()), "mask_radius_arcsec": float(args.mask_radius_arcsec), "shape": [int(ny), int(nx)], "n_masked_pixels": int(mask.sum()), "masked_fraction": float(np.mean(mask > 0)), "n_sources": int(src_rows), "track_tmin_sec": None if args.track_tmin_sec is None else float(args.track_tmin_sec), "track_tmax_sec": None if args.track_tmax_sec is None else float(args.track_tmax_sec)}
...
def cmd_apply(args: argparse.Namespace) -> int:
    ...
    if args.report_json:
        payload = {"event": str(Path(args.event).resolve()), "mask": str(Path(args.mask).resolve()), "output": str(out_path.resolve()), "input_rows": int(n_in), "removed_rows": int(n_removed), "kept_rows": int(n_in - n_removed), "removed_fraction": float(n_removed / n_in) if n_in else 0.0, "in_bounds_rows": int(n_in_bounds)}
