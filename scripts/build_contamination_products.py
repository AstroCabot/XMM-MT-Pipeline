#!/usr/bin/env python3
"""Build policy-based contamination products for comet-frame extractions.

Instead of forcing one hard science GTI, this script computes several GTI
policies and a sampled contamination timeline:

- full   : keep all times (report-only mode)
- src    : reject times when a field source overlaps the source aperture
- bkg    : reject times when a field source overlaps the background aperture
- strict : reject times when either aperture is contaminated

The selected policy is copied to ``science_gti.fits`` while the full set of GTIs
...
class TrackInterpolator:
    ...
    def at(self, t_sec: np.ndarray | float) -> tuple[np.ndarray, np.ndarray]:
        ...
        if ts.min() < self.t_sec.min() - 1e-9 or ts.max() > self.t_sec.max() + 1e-9:
...
def main() -> int:
    ...
    times = times[times <= track.obs_t1 + 1e-6]
    ...
    for sid, ra, dec in zip(srcid, src_ra, src_dec):
        ...
        reports.append(
            SourceReport(
                srcid=int(sid),
                min_sep_src_arcsec=float(np.min(dist_src)) if len(dist_src) else math.nan,
                min_sep_bkg_arcsec=float(np.min(dist_bkg)) if len(dist_bkg) else math.nan,
