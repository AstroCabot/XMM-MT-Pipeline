#!/usr/bin/env python3
"""Build contamination timeline and science GTI.

Evaluates when each detected field source overlaps the comet source and
background apertures, writes a per-sample contamination timeline CSV,
a contamination report CSV, and the output science_gti.fits.
"""
...
class TrackInterpolator:
    ...
    def at(self, t_sec: np.ndarray | float) -> tuple[np.ndarray, np.ndarray]:
        ...
        if ts.min() < self.t_sec.min() - 1e-09 or ts.max() > self.t_sec.max() + 1e-09:
            raise RuntimeError(
                "Requested interpolation time is outside the track range"
            )
...
def arcsec_offsets(
    comet_ra: np.ndarray, comet_dec: np.ndarray, src_ra: float, src_dec: float
) -> tuple[np.ndarray, np.ndarray]:
    comet = SkyCoord(
        ra=np.asarray(comet_ra, dtype=float) * u.deg,
        dec=np.asarray(comet_dec, dtype=float) * u.deg,
        frame="icrs",
    )
...
def overlaps_circle(
    src_dx: np.ndarray, src_dy: np.ndarray, ap: CircleAperture, mask_r: float
) -> np.ndarray:
...
def overlaps_annulus(
    src_dx: np.ndarray, src_dy: np.ndarray, ap: AnnulusAperture, mask_r: float
) -> np.ndarray:
    dist = np.hypot(src_dx - ap.dx, src_dy - ap.dy)
...
def overlaps(
    src_dx: np.ndarray, src_dy: np.ndarray, ap: Aperture, mask_r: float
) -> np.ndarray:
...
def good_runs_to_intervals(
    times: np.ndarray,
    good: np.ndarray,
    obs_t0: float,
    obs_t1: float,
    min_good_span_s: float,
    ...
    for i, is_good in enumerate(good):
        ...
        right = (
            float(obs_t1)
            if i == len(times) - 1
            else float(0.5 * (times[i] + times[i + 1]))
        )
...
POLICY_LABELS = {
    "full": "No contamination GTI filtering; report-only timeline",
    "src": "Reject source-aperture contamination only",
    "bkg": "Reject background-aperture contamination only",
    "strict": "Reject either source- or background-aperture contamination",
}
...
def write_gti(
    ...
    cols = fits.ColDefs(
        [
            fits.Column(name="START", format="D", unit="s", array=start),
            fits.Column(name="STOP", format="D", unit="s", array=stop),
        ]
    )
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--science-policy", default="full", choices=["full", "src", "bkg", "strict"]
    )
...
def main() -> int:
    ...
    if args.bkg_shape == "circle":
        ...
        bkg_ap: Aperture = CircleAperture(
            args.bkg_dx_arcsec, args.bkg_dy_arcsec, args.bkg_r_arcsec
        )
    else:
        if args.bkg_rin_arcsec is None or args.bkg_rout_arcsec is None:
            raise RuntimeError(
                "bkg-shape=annulus requires --bkg-rin-arcsec and --bkg-rout-arcsec"
            )
        bkg_ap = AnnulusAperture(
            args.bkg_dx_arcsec,
            args.bkg_dy_arcsec,
            args.bkg_rin_arcsec,
            args.bkg_rout_arcsec,
        )
    src_ap = CircleAperture(args.src_dx_arcsec, args.src_dy_arcsec, args.src_r_arcsec)
    times = np.arange(
        track.obs_t0,
        track.obs_t1 + 0.5 * args.sample_step_s,
        args.sample_step_s,
        dtype=float,
    )
    ...
    for sid, ra, dec in zip(srcid, src_ra, src_dec):
        ...
        reports.append(
            SourceReport(
                srcid=int(sid),
                min_sep_src_arcsec=(
                    float(np.min(dist_src)) if len(dist_src) else math.nan
                ),
                min_sep_bkg_arcsec=(
                    float(np.min(dist_bkg)) if len(dist_bkg) else math.nan
                ),
                samples_src_overlap=int(np.count_nonzero(bad_src)),
                samples_bkg_overlap=int(np.count_nonzero(bad_bkg)),
                samples_strict_overlap=int(np.count_nonzero(bad_src | bad_bkg)),
            )
        )
    good_masks = {
        ...
        "src": n_src_overlap == 0,
        "bkg": n_bkg_overlap == 0,
        "strict": (n_src_overlap == 0) & (n_bkg_overlap == 0),
    }
    gti_paths: dict[str, str] = {}
    good_time: dict[str, float] = {}
    n_intervals: dict[str, int] = {}
    for policy, good in good_masks.items():
        intervals = good_runs_to_intervals(
            times, good, track.obs_t0, track.obs_t1, float(args.min_good_span_s)
        )
        ...
        write_gti(
            ...
            notes=[
                ...
                f"Sampling step = {args.sample_step_s:.6f} s",
                f"Mask radius = {args.mask_radius_arcsec:.3f} arcsec",
            ],
        )
    ...
    with timeline_csv.open("w", newline="", encoding="utf-8") as fh:
        ...
        for t, mjd, g_full, g_src, g_bkg, g_strict, ns, nb, sids, bids in zip(
            times,
            mjd_utc,
            good_masks["full"],
            good_masks["src"],
    ...
    report_csv = (
        ...
        if args.report_csv
        else outdir / "contamination_report.csv"
    )
    with report_csv.open("w", newline="", encoding="utf-8") as fh:
        ...
        writer.writerow(
            [
                ...
                "samples_src_overlap",
                "samples_bkg_overlap",
                "samples_strict_overlap",
            ]
        )
        for rep in sorted(
            reports,
            key=lambda r: (r.samples_strict_overlap, -r.min_sep_src_arcsec),
            reverse=True,
        ):
            writer.writerow(
                [
                    rep.srcid,
                    f"{rep.min_sep_src_arcsec:.6f}",
                    f"{rep.min_sep_bkg_arcsec:.6f}",
                    rep.samples_src_overlap,
                    rep.samples_bkg_overlap,
                    rep.samples_strict_overlap,
                ]
            )
    if args.output_gti:
    ...
    summary = {
        ...
        "science_policy": args.science_policy,
        "sample_step_s": float(args.sample_step_s),
        "mask_radius_arcsec": float(args.mask_radius_arcsec),
        "observation_span_s": float(track.obs_t1 - track.obs_t0),
        "n_sources": int(len(srcid)),
        "good_time_s": good_time,
        "n_intervals": n_intervals,
        "gti_paths": gti_paths,
        "science_gti": str(science_gti),
    ...
    summary_path = (
        ...
        else outdir / "contamination_summary.json"
    )
    ...
    for policy in ["full", "src", "bkg", "strict"]:
        print(
            f"Policy {policy:6s}: good exposure {good_time[policy]:.3f} s in {n_intervals[policy]} GTI intervals"
        )
