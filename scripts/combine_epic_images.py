#!/usr/bin/env python3
"""Combine per-instrument comet-frame images into EPIC mosaics.

Adds counts and exposure maps, computes rate images, and optionally
builds a trail mask and masked rate image.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--counts", nargs="+", required=True, help="Per-instrument count images"
    )
    ap.add_argument(
        "--exposure", nargs="+", required=True, help="Per-instrument exposure maps"
    )
...
def main() -> int:
    ...
    out_summary = {
        "counts_inputs": [str(Path(p).resolve()) for p in args.counts],
        "exposure_inputs": [str(Path(p).resolve()) for p in args.exposure],
        "shape": list(counts_sum.shape),
        "finite_exposure_fraction": float(
            np.mean(np.isfinite(exp_sum) & (exp_sum > 0.0))
        ),
    }
    need_mask = bool(
        args.grid_json
        ...
        and args.srclist
        and args.out_mask
        and args.out_rate_masked
        and (args.mask_radius_arcsec > 0.0)
    )
    if need_mask:
        src_rows = 0
        try:
