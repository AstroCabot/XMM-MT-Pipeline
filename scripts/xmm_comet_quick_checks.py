...
def read_track(path: str) -> TrackData:
    with fits.open(path) as hdul:
        ...
        tab = hdu.data
        hdr = hdu.header
        delta = np.asarray(tab['DELTA'], dtype=float) if 'DELTA' in hdu.columns.names else np.full(len(tab), np.nan)
        return TrackData(mjd=np.asarray(tab['MJD'], dtype=float), ra=np.asarray(tab['RA'], dtype=float), dec=np.asarray(tab['DEC'], dtype=float), delta=delta, mjdref=float(hdr.get('MJDREF', 0.0)), timesys=str(hdr.get('TIMESYS', 'TT')), obs_t0=float(hdr.get('OBS_T0', np.nan)), obs_t1=float(hdr.get('OBS_T1', np.nan)))
...
def read_motion_segments(csv_path: str) -> list[dict[str, Any]]:
    ...
    with open(csv_path, 'r', newline='', encoding='utf-8') as fh:
        ...
        for row in reader:
            ...
            for key in ['segid', 'tstart', 'tstop', 'ra_mid_deg', 'dec_mid_deg', 'delta_mid_au', 'max_mid_offset_arcsec']:
...
def read_srclist(path: str) -> SrcList:
    with fits.open(path) as hdul:
        ...
        if tab is None or len(tab) == 0:
            return SrcList(srcid=np.asarray([], dtype=int), ra=np.asarray([], dtype=float), dec=np.asarray([], dtype=float), detml=np.asarray([], dtype=float))
        ...
        detml = np.asarray(tab[names['DET_ML']], dtype=float) if 'DET_ML' in names else None
...
def read_gti(path: str) -> GTI:
    with fits.open(path) as hdul:
        ...
        tab = hdu.data
        hdr = hdu.header
        return GTI(start=np.asarray(tab['START'], dtype=float), stop=np.asarray(tab['STOP'], dtype=float), mjdref=float(hdr.get('MJDREF', np.nan)) if 'MJDREF' in hdr else None, timesys=str(hdr.get('TIMESYS', 'TT')) if 'TIMESYS' in hdr else None)
...
def read_lightcurve(path: str, name: str) -> LightCurve:
    with fits.open(path) as hdul:
        ...
        tab = target.data
        fracexp = np.asarray(tab['FRACEXP'], dtype=float) if 'FRACEXP' in target.columns.names else None
        return LightCurve(name=name, time=np.asarray(tab['TIME'], dtype=float), rate=np.asarray(tab['RATE'], dtype=float), error=np.asarray(tab['ERROR'], dtype=float), fracexp=fracexp, header=target.header.copy())
...
def read_spectrum(path: str) -> Spectrum:
    with fits.open(path) as hdul:
        ...
        return Spectrum(channel=np.asarray(tab[chan_col], dtype=float), counts=np.asarray(tab[names['COUNTS']], dtype=float), exposure=float(exposure) if exposure is not None else None, backscal=float(backscal) if backscal is not None else None, header=target.header.copy())
...
def circle_mask_from_extent(shape: tuple[int, int], extent: tuple[float, float, float, float], dx: float, dy: float, r: float) -> np.ndarray:
    ny, nx = shape
...
def annulus_mask_from_extent(shape: tuple[int, int], extent: tuple[float, float, float, float], dx: float, dy: float, rin: float, rout: float) -> np.ndarray:
    ny, nx = shape
    xmin, xmax, ymin, ymax = extent
    xs = np.linspace(xmin, xmax, nx, endpoint=False) + 0.5 * (xmax - xmin) / nx
    ys = np.linspace(ymin, ymax, ny, endpoint=False) + 0.5 * (ymax - ymin) / ny
...
def sky_offsets_arcsec(track_ra: np.ndarray, track_dec: np.ndarray, ref_ra: float, ref_dec: float) -> tuple[np.ndarray, np.ndarray]:
    track = SkyCoord(ra=np.asarray(track_ra) * u.deg, dec=np.asarray(track_dec) * u.deg, frame='icrs')
...
def source_offsets_from_track(track_ra: np.ndarray, track_dec: np.ndarray, src_ra: float, src_dec: float) -> tuple[np.ndarray, np.ndarray]:
    comet = SkyCoord(ra=np.asarray(track_ra) * u.deg, dec=np.asarray(track_dec) * u.deg, frame='icrs')
...
def overlaps_annulus(dist: np.ndarray, rin: float, rout: float, mask_r: float) -> np.ndarray:
    return (dist + mask_r >= rin) & (dist - mask_r <= rout)
_HSTEP_RE = re.compile('^\\s*([0-9]*\\.?[0-9]+)\\s*([A-Za-z]+)?\\s*$')

def _qc_horizons_explicit_epochs(start_utc: str, stop_utc: str, step_seconds: float) -> np.ndarray:
    ...
    if times[-1] < t1:
        times = Time(np.concatenate([times.jd, np.array([t1.jd])]), format='jd', scale='utc')
    return np.unique(np.concatenate([np.array([t0.jd]), np.array(times.jd, float), np.array([t1.jd])])).astype(float)

def compare_with_horizons(env: dict[str, str], track: TrackData) -> tuple[float, float] | None:
    ...
    try:
        ...
        for i in range(0, len(epochs_jd), 80):
            obj = Horizons(id=target_id, id_type=id_type, location='500@399', epochs=[float(x) for x in epochs_jd[i:i + 80]])
            eph = obj.ephemerides()
    ...
    t_h = np.asarray((Time(mjd_h, format='mjd', scale='utc') - Time(track.mjdref, format='mjd', scale=track.timesys.lower())).to_value(u.s), dtype=float)
    t = track.t_sec
    ...
    sep = SkyCoord(ra=track.ra * u.deg, dec=track.dec * u.deg).separation(SkyCoord(ra=ra_i * u.deg, dec=dec_i * u.deg)).to_value(u.arcsec)
...
def _detect_mosaic_entries(env: dict[str, str], workdir: Path) -> list[tuple[str, Path]]:
...
def _extent_from_detect_header(header: fits.Header, shape: tuple[int, int]) -> tuple[tuple[float, float, float, float] | None, float | None, float | None]:
    if all((k in header for k in ['XMIN_AS', 'XMAX_AS', 'YMIN_AS', 'YMAX_AS', 'CTRRA', 'CTRDEC'])):
        extent = (float(header['XMIN_AS']), float(header['XMAX_AS']), float(header['YMIN_AS']), float(header['YMAX_AS']))
        return (extent, float(header['CTRRA']), float(header['CTRDEC']))
    try:
        wcs = WCS(header)
        if wcs.has_celestial:
            ny, nx = shape
            pts = np.array([[0.0, 0.0], [nx - 1.0, ny - 1.0], [0.5 * (nx - 1.0), 0.5 * (ny - 1.0)]])
            sky = wcs.celestial.pixel_to_world(pts[:, 0], pts[:, 1])
            ctr_ra = float(np.asarray(sky.ra.deg)[-1])
            ctr_dec = float(np.asarray(sky.dec.deg)[-1])
            corners = wcs.celestial.pixel_to_world([0.0, nx - 1.0, nx - 1.0, 0.0], [0.0, 0.0, ny - 1.0, ny - 1.0])
            dx, dy = sky_offsets_arcsec(np.asarray(corners.ra.deg), np.asarray(corners.dec.deg), ctr_ra, ctr_dec)
            extent = (float(np.nanmin(dx)), float(np.nanmax(dx)), float(np.nanmin(dy)), float(np.nanmax(dy)))
...
def _interp_track_to_times(track: TrackData, times: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
...
def check_clean(env: dict[str, str], outdir: Path) -> dict[str, Any]:
    ...
    skip_espfilt = env.get('CLEAN_SKIP_ESPFILT', 'no').strip().lower() in ('1', 'y', 'yes', 'true')
    ...
    for inst in ('PN', 'M1', 'M2'):
        ...
        for rf in raw_files:
            ...
            all_exposures.append({'inst': inst, 'short': short, 'status': status, 'filter': raw_filter, 'raw_ontime': raw_ontime, 'clean_ontime': display_clean_ontime if has_clean else 0.0, 'raw_rows': raw_rows, 'clean_rows': clean_rows if has_clean else 0, 'espdir': espdir})
        merged_rows = fits_rows(workdir / 'clean' / f'{inst}_clean_merged.fits')
        stats.append({'inst': inst, 'n_raw': len(raw_files), 'n_clean': len(clean_files), 'skipped': max(0, len(raw_files) - len(clean_files)), 'rows': rows, 'merged_rows': int(merged_rows) if merged_rows is not None else 0, 'raw_ontime': inst_raw_ontime, 'clean_ontime': inst_clean_ontime, 'closed_ontime': inst_closed_ontime, 'flare_ontime': inst_flare_ontime})
    ...
    for s in stats:
        ...
        if rr.size == 0:
            continue
        ax.plot(np.arange(1, len(rr) + 1), rr, marker='o', linestyle='-', label=s['inst'])
    ...
    for s in stats:
        ...
        lines.append(f"{s['inst']}: raw={s['n_raw']} clean={s['n_clean']} skipped={s['skipped']} merged_rows={s['merged_rows']}")
        if np.isfinite(med):
    ...
    savefig(fig, str(png))
    s_exposures = [e for e in all_exposures if e['short'].startswith('S') and e['espdir'].exists()]
    ...
    ax.bar(x, flare_t, width=0.55, bottom=clean_t, label='Flare-filtered', color='tomato')
    ax.bar(x, closed_t, width=0.55, bottom=clean_t + flare_t, label='Closed filter', color='gray', alpha=0.6)
    ax.set_xticks(x)
    ax.set_xticklabels(insts)
    ...
    for i, s in enumerate(stats):
        if s['raw_ontime'] > 0:
            ...
            ax.text(i, (clean_t[i] + flare_t[i] + closed_t[i]) * 1.02, f'{pct:.0f}%', ha='center', va='bottom', fontsize=9, fontweight='bold')
    ax = axes2_01
    ...
    ax = axes2_10
    tlines = []
    tlines.append(f"{'Inst':<4} {'Exp':<6} {'Status':<8} {'Filter':<8} {'Raw(ks)':>8} {'Clean(ks)':>9} {'Retained':>8} {'Raw rows':>10} {'Clean rows':>10}")
    tlines.append('-' * 95)
    for e in sorted_exp:
        raw_ks = f"{e['raw_ontime'] / 1000.0:.1f}"
        clean_ks = f"{e['clean_ontime'] / 1000.0:.1f}" if e['status'] == 'cleaned' else '--'
        ...
        tlines.append(f"{e['inst']:<4} {e['short']:<6} {e['status']:<8} {e['filter']:<8} {raw_ks:>8} {clean_ks:>9} {pct:>8} {e['raw_rows']:>10} {cr:>10}")
    tlines.append('')
    for s in stats:
        if s['raw_ontime'] > 0:
            pct = 100 * s['clean_ontime'] / s['raw_ontime']
            tlines.append(f"{s['inst']}: {s['raw_ontime'] / 1000.0:.1f} ks raw -> {s['clean_ontime'] / 1000.0:.1f} ks clean = {pct:.0f}% retained (flare: {s['flare_ontime'] / 1000.0:.1f} ks, closed: {s['closed_ontime'] / 1000.0:.1f} ks)")
    ax.text(0.01, 0.99, '\n'.join(tlines), va='top', family='monospace', fontsize=7.5, transform=ax.transAxes)
    ax.set_axis_off()
    ...
    if n_lc > 0:
        for i_lc, exp in enumerate(s_exposures[:n_lc]):
            ...
            if lc is not None:
                ...
                for gs, ge in gtis:
                    ax.axvspan((gs - t0) / 1000.0, (ge - t0) / 1000.0, color='steelblue', alpha=0.15)
                ...
                ax.set_title(f"{exp['inst']} {exp['short']} espfilt rate{title_pct}", fontsize=9)
            else:
                ax.text(0.5, 0.5, 'No rate curve', ha='center', va='center', transform=ax.transAxes)
    ...
    for s in stats:
        if s['raw_ontime'] > 0:
            retained_pcts.append(f"{s['inst']}:{100 * s['clean_ontime'] / s['raw_ontime']:.0f}%")
    return {'status': 'ok', 'png': png.name, 'extra_pngs': [png2.name], 'per_instrument': ', '.join((f"{s['inst']}:{s['n_clean']}/{s['n_raw']}" for s in stats)), 'time_retained': ', '.join(retained_pcts)}
...
def _quick_look_image(evt_path: str, pi_min: int=200, pi_max: int=12000) -> np.ndarray | None:
    try:
...
def _event_rate_histogram(evt_path: str, nbins: int=100) -> tuple[np.ndarray, np.ndarray] | None:
    try:
        with fits.open(evt_path, memmap=True) as hdul:
            ext = hdul['EVENTS'] if 'EVENTS' in hdul else hdul[1]
            data = ext.data
...
def check_exposures(env: dict[str, str], outdir: Path) -> dict[str, Any]:
    ...
    if not exposures:
        return {'status': 'missing', 'reason': 'No all_pseudoexposures.tsv found; re-run detect stage'}
    ...
    inst_colors = {'EMOS1': 'C0', 'EMOS2': 'C1', 'EPN': 'C2', 'M1': 'C0', 'M2': 'C1', 'PN': 'C2'}
    ...
    for e in exposures:
        ...
        ax_tl.barh(ypos, te - ts, left=ts, height=0.6, color=color, alpha=alpha, hatch=hatch, edgecolor='black', linewidth=0.5)
    ...
    legend_els = [Patch(facecolor='C0', alpha=0.8, label='accepted'), Patch(facecolor='C0', alpha=0.3, hatch='///', label='skipped')]
    ax_tl.legend(handles=legend_els, loc='upper right', fontsize=8)
    ax_meta = fig.add_subplot(n_rows, 2, 2)
    table_lines = [f"{'filename':<26s} {'status':<8s} {'inst':<5s} {'submode':<10s} {'datamode':<10s} {'filter':<7s} {'events':>8s} {'gti':>4s} {'ontime':>9s}", '-' * 90]
    for e in exposures:
        ontime_str = f"{e['ontime']:.1f}" if np.isfinite(e['ontime']) else '?'
        table_lines.append(f"{e.get('filename', '?'):<26s} {e.get('status', '?'):<8s} {e.get('instrument', '?'):<5s} {e.get('submode', ''):<10s} {e.get('datamode', ''):<10s} {e.get('filter', ''):<7s} {e['events']:>8d} {e['gti_rows']:>4d} {ontime_str:>9s}")
    ax_meta.text(0.01, 0.99, '\n'.join(table_lines), va='top', ha='left', family='monospace', fontsize=6, transform=ax_meta.transAxes)
        ...
        for idx in range(4):
            ...
            if idx < n_rate:
                ...
                if rh is not None:
                    ...
                    ax.set_title(f"{e.get('filename', '?')}\nevents={e['events']}", fontsize=8)
                else:
                    ax.text(0.5, 0.5, f"{e.get('filename', '?')}\nno TIME data", ha='center', va='center', fontsize=8)
                    ax.set_axis_off()
            else:
                ax.set_axis_off()
        if len(skipped_with_events) > 4:
            axes[1][3].text(0.5, 0.02, f'+{len(skipped_with_events) - 3} more skipped exposures with events', ha='center', fontsize=7, transform=axes[1][3].transAxes)
    else:
        for ax in axes[1]:
            ax.set_axis_off()
        axes[1][0].text(0.5, 0.5, 'No skipped exposures with events > 0', ha='center', va='center', fontsize=10)
    for row_idx in range(n_ql_rows):
        for col_idx in range(4):
            ...
            if ql_idx < n_ql:
                ...
                if img is not None:
                    im = ax.imshow(img, origin='lower', norm=nice_norm(img, stretch='sqrt'))
                    plt.colorbar(im, ax=ax, pad=0.01, shrink=0.8)
                    ax.set_title(f"{e.get('filename', '?')}\n{e.get('instrument', '?')} {e.get('submode', '')} {e.get('datamode', '')}", fontsize=7)
                else:
                    ax.text(0.5, 0.5, f"{e.get('filename', '?')}\nno image data", ha='center', va='center', fontsize=8)
                    ax.set_axis_off()
            else:
                ax.set_axis_off()
    fig.suptitle(f'Pseudo-exposure diagnostics: {len(accepted)} accepted, {len(skipped)} skipped ({len(skipped_with_events)} with events)', y=1.0, fontsize=12)
    png = outdir / '00b_exposures.png'
    savefig(fig, str(png))
    return {'status': 'ok', 'png': png.name, 'n_total': len(exposures), 'n_accepted': len(accepted), 'n_skipped': len(skipped), 'n_skipped_with_events': len(skipped_with_events)}

def check_track(env: dict[str, str], outdir: Path, use_horizons: bool) -> dict[str, Any]:
    workdir = Path(env['WORKDIR'])
    ...
    total_motion = 0.0
    if len(track.ra) > 1:
        sep = SkyCoord(ra=track.ra[:-1] * u.deg, dec=track.dec[:-1] * u.deg).separation(SkyCoord(ra=track.ra[1:] * u.deg, dec=track.dec[1:] * u.deg)).to_value(u.arcsec)
    ...
    if src_path.exists():
        ...
        if len(src.ra):
            ...
            nearest_source = float(np.min(comet[:, None].separation(field[None, :]).to_value(u.arcsec)))
    ...
    ax = axes[1, 0]
    if segs:
        tmid = 0.5 * (np.array([s['tstart'] for s in segs]) + np.array([s['tstop'] for s in segs]))
        ...
        ax.set_axis_off()
    ax = axes[1, 1]
    text_lines = [f'samples            : {len(track.mjd)}', f'obs span           : {duration:.1f} s', f'median track step  : {step:.3f} s' if np.isfinite(step) else 'median track step  : n/a', f'total sky motion   : {total_motion:.2f} arcsec', f'nearest detected src: {nearest_source:.2f} arcsec' if np.isfinite(nearest_source) else 'nearest detected src: n/a']
    if horiz_stats is not None:
        med_sep, max_sep = horiz_stats
        text_lines.extend([f'Horizons median Δ  : {med_sep:.4f} arcsec', f'Horizons max Δ     : {max_sep:.4f} arcsec'])
    elif use_horizons:
        text_lines.append('Horizons compare    : unavailable')
    if segs:
        blur = [float(s['max_mid_offset_arcsec']) for s in segs]
        text_lines.extend([f'segments           : {len(segs)}', f'max seg blur       : {max(blur):.3f} arcsec'])
    ax.text(0.02, 0.98, '\n'.join(text_lines), va='top', family='monospace', fontsize=9)
    ax.set_title('Track summary')
    ax.set_axis_off()
    png = outdir / '01_track.png'
    savefig(fig, str(png))
    return {'status': 'ok', 'png': png.name, 'samples': len(track.mjd), 'obs_span_s': duration, 'median_track_step_s': step, 'total_motion_arcsec': total_motion, 'nearest_detected_source_arcsec': nearest_source, 'horizons_median_arcsec': horiz_stats[0] if horiz_stats is not None else None, 'horizons_max_arcsec': horiz_stats[1] if horiz_stats is not None else None}
...
def check_detect(env: dict[str, str], outdir: Path) -> dict[str, Any]:
    ...
    track = read_track(str(workdir / 'track' / 'comet_track.fits')) if (workdir / 'track' / 'comet_track.fits').exists() else None
    fig, axes = plt.subplots(1, len(entries), figsize=(6.0 * len(entries), 5.2), squeeze=False)
    axes = axes[0]
        ...
        if extent is not None:
            ...
            if ctr_ra is not None and ctr_dec is not None:
                if src_all is not None and len(src_all.ra):
                    dx_a, dy_a = sky_offsets_arcsec(src_all.ra, src_all.dec, ctr_ra, ctr_dec)
                    ax.scatter(dx_a, dy_a, s=18, facecolors='none', edgecolors='white', linewidths=0.7, label='all detected sources')
                    if track is not None:
                        comet = SkyCoord(ra=track.ra * u.deg, dec=track.dec * u.deg)
                        field = SkyCoord(ra=src_all.ra * u.deg, dec=src_all.dec * u.deg)
                        nearest_all = float(np.min(comet[:, None].separation(field[None, :]).to_value(u.arcsec)))
                if src_cur is not None and len(src_cur.ra):
                    dx_c, dy_c = sky_offsets_arcsec(src_cur.ra, src_cur.dec, ctr_ra, ctr_dec)
                    ax.scatter(dx_c, dy_c, s=16, marker='s', facecolors='none', edgecolors='orange', linewidths=0.8, label='track-excluded list')
                    if track is not None:
                        comet = SkyCoord(ra=track.ra * u.deg, dec=track.dec * u.deg)
                        field = SkyCoord(ra=src_cur.ra * u.deg, dec=src_cur.dec * u.deg)
                        nearest_cur = float(np.min(comet[:, None].separation(field[None, :]).to_value(u.arcsec)))
                if track is not None:
                    dx_t, dy_t = sky_offsets_arcsec(track.ra, track.dec, ctr_ra, ctr_dec)
                    visible = np.isfinite(dx_t) & np.isfinite(dy_t) & (dx_t >= extent[0]) & (dx_t <= extent[1]) & (dy_t >= extent[2]) & (dy_t <= extent[3])
                    visible_frac = float(np.mean(visible)) if len(visible) else math.nan
                    n_visible = int(np.count_nonzero(visible))
                    visible_stats.append(f'{label}:{n_visible}/{len(visible)} ({visible_frac:.1%})')
                    first = True
                    for i0, i1 in split_visible_segments(visible):
                        ax.plot(dx_t[i0:i1], dy_t[i0:i1], linewidth=1.2, label='visible comet track' if first else None)
                        first = False
                    if np.any(visible):
                        idxs = np.flatnonzero(visible)
                        ax.scatter(dx_t[idxs[0]], dy_t[idxs[0]], marker='o', s=22, label='visible start')
                        ax.scatter(dx_t[idxs[-1]], dy_t[idxs[-1]], marker='s', s=22, label='visible end')
        else:
    ...
    summary_lines = [f"bands rendered      : {', '.join((label for label, _ in entries))}", f'all detected srcs   : {(len(src_all.ra) if src_all is not None else 0)}', f'track-excluded srcs : {(len(src_cur.ra) if src_cur is not None else 0)}']
    if visible_stats:
    ...
    savefig(fig, str(png))
    return {'status': 'ok', 'png': png.name, 'bands': ', '.join((label for label, _ in entries)), 'n_detected_sources': len(src_all.ra) if src_all is not None else 0, 'n_track_excluded_sources': len(src_cur.ra) if src_cur is not None else 0, 'nearest_all_source_arcsec': nearest_all, 'nearest_curated_source_arcsec': nearest_cur}

def check_image(env: dict[str, str], outdir: Path, band_label: str | None=None) -> dict[str, Any]:
    workdir = Path(env['WORKDIR'])
    bands = parse_image_bands(env.get('IMAGE_BANDS', 'soft:200:1000;broad:300:2000;hard:1000:12000'))
    if band_label:
    ...
    extent = ((x_min - center_x) * scale, (x_max - center_x) * scale, (y_min - center_y) * scale, (y_max - center_y) * scale)
    rendered: list[Path] = []
    ...
    for label, _pimin, _pimax in bands:
        ...
        src_mask = circle_mask_from_extent(rate.shape, extent, env_float(env, 'SRC_DX_ARCSEC', 0.0), env_float(env, 'SRC_DY_ARCSEC', 0.0), env_float(env, 'SRC_R_ARCSEC', 60.0))
        if env.get('BKG_MODE', 'annulus').strip().lower() == 'circle':
            bkg_mask = circle_mask_from_extent(rate.shape, extent, env_float(env, 'BKG_DX_ARCSEC', 0.0), env_float(env, 'BKG_DY_ARCSEC', 0.0), env_float(env, 'BKG_R_ARCSEC', 150.0))
        else:
            bkg_mask = annulus_mask_from_extent(rate.shape, extent, env_float(env, 'BKG_DX_ARCSEC', 0.0), env_float(env, 'BKG_DY_ARCSEC', 0.0), env_float(env, 'BKG_RIN_ARCSEC', 90.0), env_float(env, 'BKG_ROUT_ARCSEC', 150.0))
        src_exp_median = float(np.nanmedian(expo[src_mask])) if np.any(src_mask) else math.nan
        bkg_exp_median = float(np.nanmedian(expo[bkg_mask])) if np.any(bkg_mask) else math.nan
        src_rate_median = float(np.nanmedian(rate[src_mask])) if np.any(src_mask) else math.nan
        bkg_rate_median = float(np.nanmedian(rate[bkg_mask])) if np.any(bkg_mask) else math.nan
        zero_exp_frac = float(np.mean(~np.isfinite(expo) | (expo <= 0)))
        masked_frac = float(np.mean(trail_mask > 0 if trail_mask is not None else np.zeros_like(rate, dtype=bool)))
        fig, axes = plt.subplots(2, 4, figsize=(14.0, 7.8))
        top_panels = [(counts, 'EPIC counts', 'sqrt'), (expo, 'EPIC exposure', 'sqrt'), (clean if clean is not None else rate, 'EPIC clean rate' if clean is not None else 'EPIC rate', 'linear'), (net if net is not None else masked if masked is not None else rate, 'EPIC net rate' if net is not None else 'EPIC masked rate' if masked is not None else 'EPIC rate (repeat)', 'linear')]
        for ax, (data, title, stretch) in zip(axes[0], top_panels):
            if stretch == 'linear':
                ...
                if np.any(finite):
                    ...
                    vmin, vmax = (None, None)
                im = ax.imshow(data, origin='lower', extent=extent, vmin=vmin, vmax=vmax)
            else:
                im = ax.imshow(data, origin='lower', extent=extent, norm=nice_norm(data, stretch=stretch))
            plt.colorbar(im, ax=ax, pad=0.01, shrink=0.8)
            ...
            if trail_mask is not None and title.endswith('rate'):
                try:
                    ax.contour((trail_mask > 0).astype(float), levels=[0.5], origin='lower', extent=extent, linewidths=0.6)
                except Exception:
                    pass
        for ax, inst in zip(axes[1][:3], ['PN', 'M1', 'M2']):
            ...
            if np.any(finite):
                ...
                vmin, vmax = (None, None)
            im = ax.imshow(inst_rate, origin='lower', extent=extent, vmin=vmin, vmax=vmax)
            plt.colorbar(im, ax=ax, pad=0.01, shrink=0.8)
            add_apertures(ax, env)
            ax.set_xlabel('East offset (arcsec)')
            ax.set_ylabel('North offset (arcsec)')
        ...
        if trail_mask is not None:
            im = ax.imshow((trail_mask > 0).astype(float), origin='lower', extent=extent, vmin=0.0, vmax=1.0)
            plt.colorbar(im, ax=ax, pad=0.01, shrink=0.8)
            add_apertures(ax, env)
            ax.set_xlabel('East offset (arcsec)')
            ax.set_ylabel('North offset (arcsec)')
        ...
        if first_stats is None:
            first_stats = {'status': 'ok', 'png': out_png.name, 'band': label, 'src_median_exposure': src_exp_median, 'bkg_median_exposure': bkg_exp_median, 'src_median_rate': src_rate_median, 'bkg_median_rate': bkg_rate_median, 'zero_exposure_fraction': zero_exp_frac, 'trail_mask_fraction': masked_frac}
...
def check_contam(env: dict[str, str], outdir: Path) -> dict[str, Any]:
    ...
    if not timeline_path.exists() or not track_path.exists():
        return {'status': 'missing', 'reason': 'Need contamination_timeline.csv and track/comet_track.fits'}
    ...
    good_time_map = summary.get('good_time_s', {}) if isinstance(summary.get('good_time_s'), dict) else {}
    selected_good = float(good_time_map.get(science_policy, float('nan'))) if good_time_map else float('nan')
    src_list_path = Path(summary.get('srclist', workdir / 'detect' / 'field_sources_all.fits'))
    ...
    src_lookup = {int(sid): (float(ra), float(dec)) for sid, ra, dec in zip(src.srcid, src.ra, src.dec)} if src is not None else {}
    top_rows = sorted(report_rows, key=lambda r: int(r.get('samples_strict_overlap', '0') or 0), reverse=True)[:5]
    ...
    for row in top_rows:
        ...
        dist = np.hypot(dx - env_float(env, 'SRC_DX_ARCSEC', 0.0), dy - env_float(env, 'SRC_DY_ARCSEC', 0.0))
        ax.plot(t_ks, dist, label=f'src {sid}')
    ax.axhline(src_r + mask_r, linestyle='--', linewidth=1.0, label='source overlap limit')
    ...
    ax.text(0.99, 0.04, text, ha='right', va='bottom', transform=ax.transAxes, family='monospace', fontsize=8)
    png = outdir / '03_contam.png'
    savefig(fig, str(png))
    return {'status': 'ok', 'png': png.name, 'science_policy': science_policy, 'good_exposure_s': selected_good, 'top_report_rows': top_rows}
...
def check_lcurve(env: dict[str, str], outdir: Path) -> dict[str, Any]:
    ...
    def pick_lc(name: str) -> tuple[Path | None, str | None]:
        candidates = [(workdir / f'{name}_corr_abs.fits', 'abs'), (workdir / f'{name}_corr_relonly.fits', 'relonly'), (workdir / f'{name}_corr.fits', 'legacy')]
    ...
    combined = None
    combined_mode = None
    for path, mode in [(workdir / 'EPIC_total_corr_abs_lc.fits', 'abs'), (workdir / 'EPIC_total_corr_relonly_lc.fits', 'relonly'), (workdir / 'EPIC_total_corr_lc.fits', 'legacy')]:
        if path.exists():
    ...
    for lc in lcs:
        ...
        axes[0].errorbar(t_ks, lc.rate, yerr=lc.error, fmt='none', elinewidth=0.6, capsize=0)
    ...
    if combined is not None:
        ...
        axes[0].errorbar(t_ks, combined.rate, yerr=combined.error, fmt='none', elinewidth=0.6, capsize=0)
        axes[0].step(t_ks, combined.rate, where='mid', label=f'EPIC total ({combined_mode})')
        if combined.fracexp is not None:
            axes[1].step(t_ks, combined.fracexp, where='mid', label=f'EPIC total ({combined_mode})')
    ...
    out: dict[str, Any] = {'status': 'ok', 'png': png.name, 'n_instruments': len(lcs), 'instrument_modes': ', '.join((f'{k}:{v}' for k, v in sorted(modes.items()))), 'n_abs_cameras': n_abs, 'n_relonly_cameras': n_relonly, 'combined_mode': combined_mode}
    if combined is not None:
        finite = np.isfinite(combined.rate)
        out['combined_mean_rate'] = float(np.nanmean(combined.rate[finite])) if np.any(finite) else None
...
def check_spectrum(env: dict[str, str], outdir: Path) -> dict[str, Any]:
    ...
    if not src_path.exists() or not bkg_path.exists():
        return {'status': 'missing', 'reason': 'combined source/background spectra not found'}
    src = read_spectrum(str(src_path))
    bkg = read_spectrum(str(bkg_path))
    if len(src.channel) != len(bkg.channel):
        return {'status': 'error', 'reason': 'source and background spectra have different lengths'}
    scale = 1.0
    if src.exposure and bkg.exposure and src.backscal and bkg.backscal and (src.backscal != 0) and (bkg.backscal != 0):
    ...
    savefig(fig, str(png))
    return {'status': 'ok', 'png': png.name, 'src_total_counts': float(np.nansum(src.counts)), 'scaled_bkg_total_counts': float(np.nansum(scaled_bkg)), 'approx_net_total_counts': float(np.nansum(net)), 'background_scale_used': scale}
...
def check_spotcheck(env: dict[str, str], outdir: Path, band: str | None=None) -> dict[str, Any]:
    workdir = Path(env['WORKDIR'])
    band = band or env.get('QC_SPOTCHECK_BAND', '') or _first_band(env)
    root = workdir / 'qc' / 'spotchecks' / band
    if not root.exists():
        return {'status': 'missing', 'reason': f'Spot-check directory not found: {root}'}
    segdirs = sorted([p for p in root.iterdir() if p.is_dir() and p.name.startswith('seg')])
    if not segdirs:
        return {'status': 'missing', 'reason': f'No seg* directories found under {root}'}
    panels = []
    ...
    if not panels:
        return {'status': 'missing', 'reason': f'No EPIC_rate.fits files found under {root}'}
    n = len(panels)
    nrows = 2 if any((masked is not None for _seg, _img, masked, _info in panels)) else 1
    ...
    for col, (segname, img_path, masked_path, info) in enumerate(panels):
        ...
        ax.set_ylabel('Y')
        finite = np.isfinite(data)
        summary_row = {'segment': segname, 'image': str(img_path), 'finite_fraction': float(np.mean(finite)) if finite.size else None, 'median_rate': float(np.nanmedian(data[finite])) if np.any(finite) else None}
        if nrows == 2:
            ax2 = axes[1, col]
            data2, _ = read_image(str(masked_path)) if masked_path is not None else (data, None)
            ...
            summary_row['masked_image'] = str(masked_path) if masked_path is not None else None
            summary_row['masked_finite_fraction'] = float(np.mean(finite2)) if finite2.size else None
            summary_row['masked_median_rate'] = float(np.nanmedian(data2[finite2])) if np.any(finite2) else None
    ...
    json_out.write_text(json.dumps({'band': band, 'panels': summary}, indent=2) + '\n', encoding='utf-8')
    print(png)
...
def _expmap_stats(data: np.ndarray, mask: np.ndarray) -> dict[str, float | int]:
    ...
    vals = vals[finite]
    if vals.size == 0:
        return {'pixels': int(mask.sum()), 'finite_pixels': 0, 'median': float('nan'), 'mean': float('nan'), 'min': float('nan'), 'max': float('nan'), 'zero_fraction': float('nan')}
    return {'pixels': int(mask.sum()), 'finite_pixels': int(vals.size), 'median': float(np.nanmedian(vals)), 'mean': float(np.nanmean(vals)), 'min': float(np.nanmin(vals)), 'max': float(np.nanmax(vals)), 'zero_fraction': float(np.mean(vals <= 0.0))}

def check_region_support(env: dict[str, str], outdir: Path, band: str | None=None, inst: str='EPIC') -> dict[str, Any]:
    workdir = Path(env['WORKDIR'])
    ...
    src_mask = circle_mask_from_extent(data.shape, extent, env_float(env, 'SRC_DX_ARCSEC', 0.0), env_float(env, 'SRC_DY_ARCSEC', 0.0), env_float(env, 'SRC_R_ARCSEC', 60.0))
    if env.get('BKG_MODE', 'annulus').strip().lower() == 'circle':
        bkg_mask = circle_mask_from_extent(data.shape, extent, env_float(env, 'BKG_DX_ARCSEC', 0.0), env_float(env, 'BKG_DY_ARCSEC', 0.0), env_float(env, 'BKG_R_ARCSEC', 150.0))
    else:
        bkg_mask = annulus_mask_from_extent(data.shape, extent, env_float(env, 'BKG_DX_ARCSEC', 0.0), env_float(env, 'BKG_DY_ARCSEC', 0.0), env_float(env, 'BKG_RIN_ARCSEC', 90.0), env_float(env, 'BKG_ROUT_ARCSEC', 150.0))
    out = {'band': label, 'instrument': inst, 'expmap': str(path), 'source': _expmap_stats(data, src_mask), 'background': _expmap_stats(data, bkg_mask)}
    json_out = outdir / f'region_support_{inst}.json'
    json_out.write_text(json.dumps(out, indent=2, sort_keys=True) + '\n', encoding='utf-8')
    print(json.dumps(out, indent=2, sort_keys=True))
    return {'status': 'ok', 'instrument': inst, 'band': label, 'src_median_exp': out['source'].get('median'), 'bkg_median_exp': out['background'].get('median'), 'src_zero_fraction': out['source'].get('zero_fraction'), 'bkg_zero_fraction': out['background'].get('zero_fraction')}
...
def check_mosaics(env: dict[str, str], outdir: Path, band_label: str | None=None) -> dict[str, Any]:
    workdir = Path(env['WORKDIR'])
    ...
    for band in ss_bands:
        ...
        top_items = [(epic_counts, 'EPIC counts', 'sqrt'), (epic_masked, 'EPIC masked counts', 'sqrt'), (epic_bkg, 'EPIC background', 'sqrt'), (epic_exp, 'EPIC exposure', 'sqrt')]
        ...
        rendered.append(png)
        if first_stats is None:
            first_stats = {'status': 'ok', 'png': png.name, 'band': band, 'frame': 'stillsky'}
    ...
    for band in cm_bands:
        ...
        if grid_path.exists():
            ...
            scale = float(grid['scale_arcsec_per_phys'])
            comet_extent = ((xmin - cx) * scale, (xmax - cx) * scale, (ymin - cy) * scale, (ymax - cy) * scale)
        fig, axes = plt.subplots(2, 4, figsize=(16.0, 8.0))
        ...
        rendered.append(png)
        if first_stats is None:
            first_stats = {'status': 'ok', 'png': png.name, 'band': band, 'frame': 'comet'}
    if not rendered or first_stats is None:
...
def write_report(path: Path, env: dict[str, str], results: dict[str, dict[str, Any]]) -> None:
    lines: list[str] = []
    ...
    lines.append('')
    order = [('clean', 'Cleaning / flare filtering'), ('exposures', 'Pseudo-exposure diagnostics'), ('track', 'Track / ephemeris'), ('detect', 'Still-sky detection'), ('contam', 'Contamination GTI'), ('image', 'Comet-frame imaging'), ('lcurve', 'Light curves'), ('spectrum', 'Spectrum'), ('mosaics', 'QC mosaics (still-sky + comet)'), ('spotcheck', 'Comet-frame spot-checks'), ('region-support', 'Region exposure support')]
    for key, title in order:
        ...
        if res.get('top_report_rows'):
            ...
            for row in res['top_report_rows']:
                lines.append(f"- srcid={row.get('srcid')} min_sep_src={row.get('min_sep_src_arcsec')} min_sep_bkg={row.get('min_sep_bkg_arcsec')} src_overlap={row.get('samples_src_overlap')} bkg_overlap={row.get('samples_bkg_overlap')} strict_overlap={row.get('samples_strict_overlap')}")
        lines.append('')
    path.write_text('\n'.join(lines).rstrip() + '\n', encoding='utf-8')
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument('--outdir', default='', help='Output QA directory (default: WORKDIR/qc)')
    ap.add_argument('--band', help='Image band label for image/spotcheck/region-support checks (default: first IMAGE_BANDS entry)')
    ap.add_argument('--inst', default='EPIC', choices=['EPIC', 'PN', 'M1', 'M2'], help='Instrument for region-support check')
    ap.add_argument('--horizons', action='store_true', help='Compare stored track against a fresh Horizons query when possible')
    ap.add_argument('checks', nargs='+', choices=['clean', 'exposures', 'track', 'detect', 'image', 'contam', 'lcurve', 'spectrum', 'mosaics', 'spotcheck', 'region-support', 'all'])
...
def main() -> int:
    ...
    checks = args.checks
    if 'all' in checks:
        checks = ['clean', 'exposures', 'track', 'detect', 'contam', 'image', 'lcurve', 'spectrum', 'mosaics', 'spotcheck', 'region-support']
    ...
    for check in checks:
        if check == 'clean':
            ...
            results[check] = check_region_support(env, outdir, band=args.band, inst=args.inst)
        else:
