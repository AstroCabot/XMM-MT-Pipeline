#!/usr/bin/env python3
"""Quick-look QA checks for the XMM comet science pipeline outputs.

This script is intentionally read-only: it inspects products already created by
the comet pipeline and writes PNG + Markdown summaries.

Main checks
-----------
track   : ephemeris / trajectory sanity, optional Horizons comparison

detect  : still-sky soft/hard diagnostic mosaics with all-vs-curated source
          overlays and comet track overlay

image   : comet-frame counts / exposure / rate / masked-rate panels with
          source/background aperture overlays

contam  : contamination-GTI sanity: nearest-source separations and GTI summary

lcurve  : corrected per-instrument and combined EPIC light curves

spectrum: combined source / scaled background / approximate net spectrum

all     : run all available checks and build one Markdown report
"""
...
try:  # optional only for --horizons checks
...
def load_shell_env(path: str) -> dict[str, str]:
    """Load a shell-style env file via bash, preserving shell expansions."""
    quoted = shlex.quote(str(path))
    cmd = [
        "bash",
        "-lc",
        f"set -a; source {quoted} >/dev/null 2>&1; env -0",
    ]
...
# -----------------------------------------------------------------------------
# Optional Horizons comparison
# -----------------------------------------------------------------------------


_HSTEP_RE = re.compile(r"^\s*([0-9]*\.?[0-9]+)\s*([A-Za-z]+)?\s*$")
...
def compare_with_horizons(
    ...
    # interpolate Horizons onto stored track times in Cartesian space for wrap safety
...
def _extent_from_detect_header(
    ...
    try:
        ...
        if wcs.has_celestial:
            ...
            # Approximate extent via corner offsets around centre.
...
def _read_events_header(path: Path) -> dict[str, Any]:
    """Read ONTIME, LIVETIME, FILTER, TSTART, TSTOP and row count from EVENTS."""
    info: dict[str, Any] = {}
    if not path.exists():
        return info
    try:
...
def _read_espfilt_lc(espdir: Path) -> tuple[np.ndarray, np.ndarray] | None:
    """Read the FOV rate light curve from an espfilt working directory."""
...
def _read_espfilt_gti(espdir: Path) -> list[tuple[float, float]]:
    """Read GTI intervals from an espfilt working directory."""
...
def check_clean(env: dict[str, str], outdir: Path) -> dict[str, Any]:
    ...
    for inst in ("PN", "M1", "M2"):
        ...
        for rf in raw_files:
            ...
            stem = rfp.stem  # e.g. 4759_0963720201_EPN_S003_ImagingEvts

            # Derive clean file path
    ...
    # ================================================================
    # Figure 1 — original overview (rows/multiplicity)
    # ================================================================
    ...
    # ================================================================
    # Figure 2 — time budget & flare diagnostics
    # ================================================================
    # Identify main scheduled (S-series) exposures for rate curves
    s_exposures = [
    ...
    for e in sorted_exp:
        ...
        raw_vals.append(e["raw_ontime"] / 1e3)
        clean_vals.append(e["clean_ontime"] / 1e3)
    ...
    tlines.append("-" * 95)
    for e in sorted_exp:
        raw_ks = f"{e['raw_ontime']/1e3:.1f}"
        clean_ks = f"{e['clean_ontime']/1e3:.1f}" if e["status"] == "cleaned" else "--"
    ...
    for s in stats:
        if s["raw_ontime"] > 0:
            pct = 100 * s["clean_ontime"] / s["raw_ontime"]
            tlines.append(
                f"{s['inst']}: {s['raw_ontime']/1e3:.1f} ks raw -> {s['clean_ontime']/1e3:.1f} ks clean = {pct:.0f}% retained "
                f"(flare: {s['flare_ontime']/1e3:.1f} ks, closed: {s['closed_ontime']/1e3:.1f} ks)"
            )
    ax.text(
        0.01,
        0.99,
    ...
    if n_lc > 0:
        for i_lc, exp in enumerate(s_exposures[:n_lc]):
            ...
            if lc is not None:
                ...
                # Bin to 100s for cleaner display
                bin_s = 100
                ...
                if n_bins > 5:
                    ...
                    ax.plot((t_mid - t0) / 1e3, binned, linewidth=0.8, color="k")
                # Shade accepted GTI intervals
                for gs, ge in gtis:
                    ax.axvspan(
...
# ---------------------------------------------------------------------------
# check_exposures — pseudo-exposure QC diagnostics
# ---------------------------------------------------------------------------


def _read_exposure_tsv(path: Path) -> list[dict[str, str]]:
    """Read a TSV manifest written by stage_detect into a list of dicts."""
    if not path.exists():
        return []
    import csv as _csv
...
def _quick_look_image(
    ...
    """Bin events into a quick-look sky image (RA/DEC or X/Y)."""
    try:
        with fits.open(evt_path, memmap=True) as hdul:
            ...
            if not np.any(mask):
                return None
            # Try detector coordinates X, Y (SAS convention)
...
def _event_rate_histogram(
    ...
    """Build a count-rate vs time histogram from an event list (all PI)."""
    try:
        with fits.open(evt_path, memmap=True) as hdul:
            ext = hdul["EVENTS"] if "EVENTS" in hdul else hdul[1]
            data = ext.data
...
def check_exposures(env: dict[str, str], outdir: Path) -> dict[str, Any]:
    """QC diagnostic for all pseudo-exposures (accepted + skipped).

    Reads the comprehensive manifest written by stage_detect to show:
    - observation timeline (TSTART/TSTOP per exposure, colour by status)
    - instrument / mode / submode / filter for every exposure
    - quick-look images and rate curves for skipped files with events > 0
    """
    workdir = Path(env["WORKDIR"])
    ...
    # Determine number of figure rows needed
    # Row 0: timeline + metadata table (always)
    # Row 1: rate histograms for skipped exposures with events (if any)
    # Rows 2+: quick-look images for skipped exposures (up to 6 per row)
    n_ql = len(skipped_with_events)
    n_ql_rows = max(0, (n_ql + 3) // 4)  # up to 4 per row
    n_rows = 2 + n_ql_rows
    ...
    # --- Row 0: observation timeline and metadata summary ---

    # Timeline panel (left half of row 0)
    ...
    # Metadata summary table (right half of row 0)
    ax_meta = fig.add_subplot(n_rows, 2, 2)
    table_lines = [
        f"{'filename':<26s} {'status':<8s} {'inst':<5s} {'submode':<10s} "
        f"{'datamode':<10s} {'filter':<7s} {'events':>8s} {'gti':>4s} {'ontime':>9s}",
        "-" * 90,
    ]
    for e in exposures:
        ontime_str = f"{e['ontime']:.1f}" if np.isfinite(e["ontime"]) else "?"
        table_lines.append(
            f"{e.get('filename','?'):<26s} {e.get('status','?'):<8s} "
            f"{e.get('instrument','?'):<5s} {e.get('submode',''):<10s} "
            f"{e.get('datamode',''):<10s} {e.get('filter',''):<7s} "
            f"{e['events']:>8d} {e['gti_rows']:>4d} {ontime_str:>9s}"
        )
    ax_meta.text(
        0.01,
        0.99,
    ...
    # Hide the original subplot axes for row 0 (we used add_subplot above)
    for ax in axes[0]:
        ax.set_visible(False)

    # --- Row 1: rate histograms for skipped-with-events ---
    ...
    # --- Rows 2+: quick-look images for skipped exposures with events ---
        ...
        for col_idx in range(4):
            ...
            if ql_idx < n_ql:
                ...
                if img is not None:
                    ...
                    ax.set_title(
                        f"{e.get('filename','?')}\n{e.get('instrument','?')} "
                        f"{e.get('submode','')} {e.get('datamode','')}",
                        fontsize=7,
                    )
                else:
                    ax.text(
    ...
    fig.suptitle(
        f"Pseudo-exposure diagnostics: {len(accepted)} accepted, {len(skipped)} skipped "
        f"({len(skipped_with_events)} with events)",
        y=1.0,
        fontsize=12,
    )
...
def check_image(
    ...
    for label, _pimin, _pimax in bands:
        ...
        top_panels = [
            ...
            (rate, "EPIC rate", "linear"),
            (
                masked if masked is not None else rate,
                "EPIC masked rate" if masked is not None else "EPIC rate (repeat)",
                "linear",
            ),
        ]
        ...
        if trail_mask is not None:
            ...
            text = (
                f"band={label}\n"
                f"src median exp  : {src_exp_median:.4g}\n"
                f"bkg median exp  : {bkg_exp_median:.4g}\n"
                f"src median rate : {src_rate_median:.4g}\n"
                f"bkg median rate : {bkg_rate_median:.4g}\n"
                f"zero-exp frac   : {zero_exp_frac:.3%}"
            )
...
# ---------------------------------------------------------------------------
# check: spotcheck — render first/middle/last comet-frame rate montage
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# check_mosaics — QC mosaic diagnostics
# ---------------------------------------------------------------------------


def _mosaic_extent(header: fits.Header, shape: tuple[int, int]):
    """Return (extent, center_ra, center_dec) for a mosaic image.

    Works for both make_stillsky_mosaic custom headers (XMIN_AS etc.) and
    standard SAS WCS (evselect-produced) images.
    """
...
def check_mosaics(
    ...
    """Render QC diagnostic panels for still-sky and comet-frame mosaic products."""
    workdir = Path(env["WORKDIR"])
    ...
    for band in ss_bands:
        ...
        # Find EPIC combined products first
        ...
        # Row 1: EPIC counts, masked counts, background, exposure
        # Row 2: PN rate, M1 rate, M2 rate, source mask
        ...
        # Bottom row: per-instrument rate images + source mask
    ...
    for band in cm_bands:
        ...
        # Grid extent from WORKDIR/images/grid.json
        grid_path = workdir / "images" / "grid.json"
        comet_extent = None
        if grid_path.exists():
            grid = json.loads(grid_path.read_text(encoding="utf-8"))
        ...
        # Row 1: PN/M1/M2 rate + EPIC rate
        # Row 2: PN/M1/M2 background + EPIC rate_masked
        fig, axes = plt.subplots(2, 4, figsize=(16.0, 8.0))
        ...
        # Bottom row: per-inst background + masked rate
        for ax, inst in zip(axes[1][:3], instruments):
    ...
    for key, title in order:
        ...
        if res.get("top_report_rows"):
            ...
            for row in res["top_report_rows"]:
                lines.append(
                    f"- srcid={row.get('srcid')} min_sep_src={row.get('min_sep_src_arcsec')} "
                    f"min_sep_bkg={row.get('min_sep_bkg_arcsec')} src_overlap={row.get('samples_src_overlap')} "
                    f"bkg_overlap={row.get('samples_bkg_overlap')} strict_overlap={row.get('samples_strict_overlap')}"
                )
        lines.append("")
