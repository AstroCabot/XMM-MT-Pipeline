#!/usr/bin/env python3
"""Build a comet track FITS and motion-limited segment list.

Outputs
-------
1) track FITS with columns MJD, RA, DEC, DELTA suitable for SAS moving-target tasks.
2) CSV segment table with segment midpoint coordinates and observation times.

The segmentation is purely motion-based in the sky frame. For each candidate
interval [t0, t1], the code computes the sky position at the interval midpoint
and requires that the maximum angular offset of the sampled comet track from
that midpoint remain <= max_blur_arcsec.

Horizons note
-------------
The JPL Horizons system does not accept the "s" (seconds) unit in STEP_SIZE for
observer ephemerides. When the requested cadence is sub-minute (e.g. 30s), this
script queries an explicit list of UTC epochs in small chunks instead of using a
range dictionary.
"""
...
except Exception:  # pragma: no cover - optional only when user supplies track file
    Horizons = None
...
_STEP_RE = re.compile(r"^\s*([0-9]*\.?[0-9]+)\s*([A-Za-z]+)?\s*$")


def _parse_fixed_step_seconds(step: str) -> tuple[float | None, str | None]:
    """Return (step_seconds, canonical_unit) for fixed-cadence strings.

    canonical_unit is one of 's', 'm', 'h', 'd', 'mo', 'y'. Returns (None, None)
    when the input should be passed through unchanged to Horizons.
    """
    s = step.strip()
    m = _STEP_RE.match(s)
    if not m:
    ...
    if unit == "":
        return None, None  # bare integer -> Horizons "number of intervals" syntax
...
def _build_epoch_list(start_utc: str, stop_utc: str, step_seconds: float) -> np.ndarray:
    ...
    # Ensure start and exact stop are present, unique, and ordered.
    jd = np.unique(
        np.concatenate(
...
def _parse_eph_table(eph) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    cols = eph.colnames
    # racol = _find_col_case_insensitive(cols, ["RA"])
    # deccol = _find_col_case_insensitive(cols, ["DEC"])
...
def _query_horizons_epoch_chunk(
    target_id: str,
    id_type: str,
    epoch_chunk_jd: np.ndarray,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    obj = Horizons(
        id=target_id,
        id_type=id_type,
        location="500@399",  # explicit Earth geocenter for moving-target track semantics
...
def query_horizons_track(
    ...
    # JPL Horizons observer ephemerides do not support STEP_SIZE with seconds units.
    # For sub-minute cadences, or any fixed cadence that is more naturally expressed
    # in seconds, query explicit epochs in chunks.
    use_explicit_epochs = bool(step_seconds is not None and step_seconds < 60.0)

    # Also use explicit epochs for non-integer minute/hour/day cadences, because
    # Horizons STEP_SIZE expects integer steps in those calendar/time units.
    if step_seconds is not None and canonical_unit in {"m", "h", "d"}:
        ...
        use_explicit_epochs = use_explicit_epochs or (
            abs(step_seconds / base - round(step_seconds / base)) > 1e-12
        )
    ...
    if use_explicit_epochs:
        ...
        # For integer minute/hour/day/month/year steps, use the compact range query.
        # Pad stop by one cadence for minute/hour/day steps so the returned track fully
        # brackets the observation even when the endpoint is not aligned to the cadence.
        ...
        obj = Horizons(
            id=target_id,
            id_type=id_type,
            location="500@399",  # explicit Earth geocenter
            epochs={
    ...
    # Drop any duplicate times at chunk boundaries and sort.
...
class TrackInterpolator:
    ...
    def max_offset_from_midpoint_arcsec(self, t0: float, t1: float) -> float:
        ...
        # Always include the exact endpoints and midpoint.
...
def build_segments(
    ...
    if snap_s <= 0:
        # Fallback to track sampling cadence.
    ...
    # Boundaries are defined on a global grid rooted at obs_tstart.
    while cur < obs_tstop - tol:
        ...
        if chosen_end is None:
            ...
            # If the remaining interval is shorter than min_seg_s, merge it
            # into the previous segment rather than failing.
            if remaining < min_seg_s and segments:
                ...
                break
            # If there is no previous segment, accept the short remainder as-is.
            ...
            raise RuntimeError(
                "Cannot satisfy segmentation constraints: one snapped interval of "
                f"{test_end - cur:.3f} s already has midpoint blur {test_blur:.3f} arcsec, "
                f"exceeding max_blur_arcsec={max_blur_arcsec:.3f}. "
                "Increase MAX_SEG_BLUR_ARCSEC or reduce SEGMENT_SNAP_S / LC_BIN_S."
            )
...
def main() -> int:
    ...
    if args.track_input:
        ...
        # Pad the query window so the track fully brackets the observation
        # even after the UTC -> timesys (TT) conversion.
    ...
    # Convert track times to XMM-style seconds for interpolation/segmentation.
    # Horizons MJD is in UTC; mjdref is in timesys (typically TT).  Use astropy
    # Time arithmetic so the TT-UTC offset (~69 s) is handled correctly.
    ...
    if t_sec[0] > obs_tstart or t_sec[-1] < obs_tstop:
        raise RuntimeError(
            "Track does not fully cover the observation. "
            f"Track spans [{t_sec[0]:.3f}, {t_sec[-1]:.3f}] s but observation spans [{obs_tstart:.3f}, {obs_tstop:.3f}] s."
        )
