...
def xmm_seconds_to_time(seconds: np.ndarray | float, mjdref: float, timesys: str) -> Time:
    return Time(mjdref, format='mjd', scale=timesys.lower()) + TimeDelta(seconds, format='sec')
...
def _find_col_case_insensitive(names: Iterable[str], candidates: Iterable[str]) -> str | None:
...
def load_track_from_csv(csv_file: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    if not (tcol and racol and deccol and dcol):
        raise RuntimeError('Track CSV must contain columns time_iso, ra_deg, dec_deg, delta_au (case-insensitive).')
    tt = Time(np.array(data[tcol]), format='isot', scale='utc')
    return (tt.mjd.astype(float), np.array(data[racol], float), np.array(data[deccol], float), np.array(data[dcol], float))

def load_track_from_fits(fits_file: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    with fits.open(fits_file) as hdul:
        ...
        return (np.array(tab[mjdcol], float), np.array(tab[racol], float), np.array(tab[deccol], float), np.array(tab[dcol], float))
...
def _build_epoch_list(start_utc: str, stop_utc: str, step_seconds: float) -> np.ndarray:
    ...
    jd = np.unique(np.concatenate([np.array([t0.jd]), np.array(times.jd, float), np.array([t1.jd])]))
...
def _parse_eph_table(eph) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    if not (racol and deccol and dcol and tcol):
        raise RuntimeError(f'Horizons output missing one of RA/DEC/delta/datetime_jd columns; got {cols}')
...
def _query_horizons_epoch_chunk(target_id: str, id_type: str, epoch_chunk_jd: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    obj = Horizons(id=target_id, id_type=id_type, location='500@399', epochs=[float(x) for x in epoch_chunk_jd])
    eph = obj.ephemerides(quantities='2,20', extra_precision=True)
    return _parse_eph_table(eph)

def query_horizons_track(target_id: str, id_type: str, start_utc: str, stop_utc: str, step: str, epoch_chunk_size: int=40) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    if Horizons is None:
        raise RuntimeError('astroquery is unavailable, but no external track file was supplied.')
    ...
    if step_seconds is not None and canonical_unit in {'m', 'h', 'd'}:
        ...
        use_explicit_epochs = use_explicit_epochs or abs(step_seconds / base - round(step_seconds / base)) > 1e-12
    if use_explicit_epochs:
        epochs_jd = _build_epoch_list(start_utc=start_utc, stop_utc=stop_utc, step_seconds=float(step_seconds))
        ...
        for i in range(0, len(epochs_jd), int(epoch_chunk_size)):
            ...
            mjd_i, ra_i, dec_i, delta_i = _query_horizons_epoch_chunk(target_id=target_id, id_type=id_type, epoch_chunk_jd=chunk)
        ...
        if step_seconds is not None and canonical_unit in {'m', 'h', 'd'}:
            t1 = Time(stop_utc, format='isot', scale='utc') + TimeDelta(float(step_seconds), format='sec')
            stop_for_query = t1.isot
            qval = int(round(step_seconds / {'m': 60.0, 'h': 3600.0, 'd': 86400.0}[canonical_unit]))
            query_step = f'{qval}{canonical_unit}'
        obj = Horizons(id=target_id, id_type=id_type, location='500@399', epochs={'start': start_utc.replace('T', ' '), 'stop': stop_for_query.replace('T', ' '), 'step': query_step})
...
class TrackInterpolator:
    ...
    @classmethod
    def from_track(cls, t_sec: np.ndarray, ra_deg: np.ndarray, dec_deg: np.ndarray, delta_au: np.ndarray) -> 'TrackInterpolator':
        sc = SkyCoord(ra=ra_deg * u.deg, dec=dec_deg * u.deg, frame='icrs')
        xyz = sc.cartesian.xyz.value.T.astype(float)
        return cls(t_sec=np.array(t_sec, float), xyz=xyz, delta_au=np.array(delta_au, float))

    def at(self, t_sec: float | np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def build_segments(interp: TrackInterpolator, obs_tstart: float, obs_tstop: float, max_blur_arcsec: float, snap_s: float, min_seg_s: float) -> list[tuple[int, float, float, float, float, float, float]]:
    ...
    while cur < obs_tstop - tol:
        ...
        if chosen_end is None:
            ...
            if remaining < min_seg_s and segments:
                ...
                segments[-1] = (prev[0], prev[1], new_end, float(ra_m), float(dec_m), float(delta_m), float(new_blur))
                break
            if remaining < min_seg_s and (not segments):
                ...
                segments.append((segid, cur, obs_tstop, float(ra_mid), float(dec_mid), float(delta_mid), float(blur)))
                break
            test_end = min(cur + snap_s, obs_tstop)
            test_blur = interp.max_offset_from_midpoint_arcsec(cur, test_end)
            raise RuntimeError(f'Cannot satisfy segmentation constraints: one snapped interval of {test_end - cur:.3f} s already has midpoint blur {test_blur:.3f} arcsec, exceeding max_blur_arcsec={max_blur_arcsec:.3f}. Increase MAX_SEG_BLUR_ARCSEC or reduce SEGMENT_SNAP_S / LC_BIN_S.')
        tmid = 0.5 * (cur + chosen_end)
        ra_mid, dec_mid, delta_mid = interp.at(tmid)
        segments.append((segid, cur, chosen_end, float(ra_mid), float(dec_mid), float(delta_mid), float(chosen_blur)))
        segid += 1
        cur = chosen_end
    return segments

def write_track_fits(out_file: str, mjd: np.ndarray, ra: np.ndarray, dec: np.ndarray, delta: np.ndarray, obs_tstart: float, obs_tstop: float, mjdref: float, timesys: str, source_desc: str) -> None:
    cols = fits.ColDefs([fits.Column(name='MJD', format='D', unit='d', array=np.array(mjd, dtype=np.float64)), fits.Column(name='RA', format='E', unit='deg', array=np.array(ra, dtype=np.float32)), fits.Column(name='DEC', format='E', unit='deg', array=np.array(dec, dtype=np.float32)), fits.Column(name='DELTA', format='E', unit='AU', array=np.array(delta, dtype=np.float32))])
...
def write_segments_csv(out_file: str, segments: list[tuple[int, float, float, float, float, float, float]], mjdref: float, timesys: str) -> None:
    fields = ['segid', 'tstart', 'tstop', 'ra_mid_deg', 'dec_mid_deg', 'delta_mid_au', 'max_mid_offset_arcsec', 'utc_start', 'utc_stop']
    with open(out_file, 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(fields)
        for segid, t0, t1, ra, dec, delta, blur in segments:
            w.writerow([segid, f'{t0:.6f}', f'{t1:.6f}', f'{ra:.10f}', f'{dec:.10f}', f'{delta:.8f}', f'{blur:.6f}', xmm_seconds_to_utc_iso(t0, mjdref, timesys), xmm_seconds_to_utc_iso(t1, mjdref, timesys)])

def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser()
    ap.add_argument('--reference-event', required=True, help='EPIC event file used to define TIME range and MJDREF')
    ap.add_argument('--track-out', required=True, help='Output FITS file for movecalc track')
    ap.add_argument('--segments-out', required=True, help='Output CSV file for motion-limited segments')
    ap.add_argument('--track-input', help='Optional input comet track (CSV or FITS) with time/RA/DEC/DELTA')
    ap.add_argument('--target-id', help='Horizons target ID if no track-input is supplied')
    ap.add_argument('--id-type', default='smallbody', help='Horizons id_type (default: smallbody)')
    ap.add_argument('--step', default='1m', help='Track sampling, e.g. 30s, 1m, 5m. Sub-minute uses explicit epochs.')
    ap.add_argument('--max-blur-arcsec', type=float, required=True, help='Maximum allowed comet offset from segment midpoint')
    ap.add_argument('--snap-s', type=float, default=0.0, help='Snap segment boundaries to this cadence in seconds')
    ap.add_argument('--min-seg-s', type=float, default=0.0, help='Minimum segment duration in seconds')
...
def main() -> int:
    ...
    if args.track_input:
        ...
        mjd, ra, dec, delta = query_horizons_track(target_id=args.target_id, id_type=args.id_type, start_utc=padded_start, stop_utc=padded_stop, step=args.step)
    ...
    if t_sec[0] > obs_tstart or t_sec[-1] < obs_tstop:
        raise RuntimeError(f'Track does not fully cover the observation. Track spans [{t_sec[0]:.3f}, {t_sec[-1]:.3f}] s but observation spans [{obs_tstart:.3f}, {obs_tstop:.3f}] s.')
    interp = TrackInterpolator.from_track(t_sec=t_sec, ra_deg=ra, dec_deg=dec, delta_au=delta)
    segments = build_segments(interp=interp, obs_tstart=obs_tstart, obs_tstop=obs_tstop, max_blur_arcsec=float(args.max_blur_arcsec), snap_s=float(args.snap_s), min_seg_s=float(args.min_seg_s))
    ...
    write_track_fits(out_file=args.track_out, mjd=mjd, ra=ra, dec=dec, delta=delta, obs_tstart=obs_tstart, obs_tstop=obs_tstop, mjdref=mjdref, timesys=timesys, source_desc=source_desc)
    write_segments_csv(out_file=args.segments_out, segments=segments, mjdref=mjdref, timesys=timesys)
    ...
    if segments:
        ...
        print(f'Segment duration s:   min={min(durs):.3f} med={np.median(durs):.3f} max={max(durs):.3f}')
        print(f'Midpoint blur arcsec: min={min(blurs):.3f} med={np.median(blurs):.3f} max={max(blurs):.3f}')
    return 0
