#!/usr/bin/env python3
"""Stage-level manifest, QC index, and repro diagnostics for the XMM comet package.

Subcommands
-----------
manifest : Write a compact JSON, Markdown, and overview PNG showing whether each
           major stage produced the expected files.
index    : Build a stage-by-stage QC navigation index (STAGE_QC.md).
repro    : Report how many calibrated EPIC event-list candidates exist per
...
def cmd_manifest(args: argparse.Namespace) -> int:
    ...
    stages["spectrum"] = {
        "status": stage_status(spec_ok),
        "combined_src": str(workdir / "spectra" / "EPIC_src_combined.fits"),
        "combined_grp": str(workdir / "spectra" / "EPIC_src_combined_grp.fits"),
    }

    # merge
    merge_ok = nonempty(workdir / "final" / "EPIC_comet_merged_sciencegti.fits")
    stages["merge"] = {
        "status": stage_status(merge_ok),
        "science_eventlist": str(
            workdir / "final" / "EPIC_comet_merged_sciencegti.fits"
        ),
        "full_eventlist": str(workdir / "final" / "EPIC_comet_merged_full.fits"),
    }
    ...
    txt = [
        ...
        f"merged rows : PN={clean_data['PN']['merged_rows']} M1={clean_data['M1']['merged_rows']} M2={clean_data['M2']['merged_rows']}",
    ]
    ax.text(
        0.99,
        0.98,
...
def cmd_index(args: argparse.Namespace) -> int:
    ...
    lines = [
        ...
        f"Workdir: `{workdir}`",
        "",
        "This file is intended to be the one place to inspect after a run. Each section points to the main image and/or text artifact for that stage.",
        "",
        "## Summary table",
        "",
    ...
    mapping = {
        "init": ["00_manifest.md"],
        "repro": ["00_manifest.md", "00_manifest.png"],
        "clean": ["00_clean.png", "00_manifest.md", "00_manifest.png"],
        "track": ["01_track.png", "qc_report.md"],
        "detect": ["02_detect.png", "00_manifest.md"],
        "comet": [
            f"07_spotchecks_{env.get('QC_SPOTCHECK_BAND', '') or env.get('IMAGE_BANDS', 'broad:300:2000').split(';')[0].split(':')[0]}.png",
            "00_manifest.md",
        ],
        "contam": ["04_contam.png", "qc_report.md"],
        "image": ["03_image.png", "qc_report.md"],
        "lcurve": ["05_lcurve.png", "region_support_PN.json", "qc_report.md"],
        "spectrum": ["06_spectrum.png", "qc_report.md"],
        "merge": ["00_manifest.md"],
    }
    ...
    lines += [
        "### init / repro / clean",
        "Use `00_manifest.md` and `00_manifest.png` to confirm that all raw event lists were found, cleaned, and merged per instrument. For a multi-pointing observation, the raw counts should exceed one per instrument if the ODF really contains multiple pointings.",
        "",
        "### track",
        "Inspect `01_track.png`. The track should be smooth, and the motion-segmentation curve should not show pathological jumps except possibly a small final-segment remainder effect.",
        "",
        "### detect",
        "Inspect `02_detect.png`. The visible portion of the comet track should lie over the detect mosaics, and the soft/hard panels should show that source overlays track real sky structure rather than a clipped or shifted footprint. Compare the full detected-source list against the more conservative track-excluded list.",
        "",
        "### comet",
        "Inspect the spot-check montage `07_spotchecks_*.png`. In a good comet frame the comet stays compact while stationary field sources trail. This is the single most important visual sanity check for the moving-target reprojection.",
        "",
        "### contam",
        "Inspect `04_contam.png` and the contamination section in `qc_report.md`. The policy rows should make it obvious how much exposure would be lost by source-only, background-only, or strict contamination cuts, and the overlap-count panel should show whether masking is a better choice than discarding whole intervals.",
        "",
        "### image",
        "Inspect `03_image.png`. The counts, exposure, rate, and masked-rate panels should all be finite in the comet-centered footprint; masked-rate images should suppress obvious trail contamination.",
        "",
        "### lcurve",
        "Inspect `05_lcurve.png`, `lcurve/lc_summary.tsv`, and the `region_support_*.json` files. The strict default is that absolute-correction failures stop the pipeline unless you explicitly allow a rel-only fallback.",
        "",
        "### spectrum",
        "Inspect `06_spectrum.png`. The combined source spectrum should sit above the scaled background over at least part of the configured band, and the approximate net panel should not be dominated by wild oscillations from a broken background selection.",
        "",
    ]

    # Include region-support snippets if present.
    ...
    for inst, path in region_reports.items():
        ...
        lines.append(
            f"- **{inst}**: source median exp={src_med}, background median exp={bkg_med}, source zero-fraction={src_zero}, background zero-fraction={bkg_zero}"
        )
    lines.append("")
    ...
    if report_md.exists():
        ...
        lines.append("")
        lines.append(
            "The companion `qc_report.md` contains the detailed stage-level measurements produced by `xmm_comet_quick_checks.py`."
        )
        lines.append("")
