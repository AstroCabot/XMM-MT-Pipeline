#!/usr/bin/env python3
"""
PPS vs Pipeline side-by-side comparison.

Compares GTIs, flare filtering, event counts, source detection,
and attitude between official PPS/MT_PPS products and our pipeline output.

Usage:
  python3 scripts/pps_vs_pipeline_compare.py
"""
...
def event_count_fast(filepath):
    """Get event count from NAXIS2 header (avoids decompressing all data)."""
...
def main():
    workdir = Path(
        "/mnt/c/Users/cabot/OneDrive/Documents/Research/Cambridge/X3I/"
        "reduction_v4/output"
    )
    mt_dir = Path(
        "/mnt/c/Users/cabot/OneDrive/Documents/Research/Data/"
        "xmm_3iatlas_updated/MT_PPS"
    )
    outdir = Path(
        "/mnt/c/Users/cabot/OneDrive/Documents/Research/Cambridge/X3I/"
        "reduction_v4/xmm_comet_pipeline_refactor_release/pps_analytics_output"
    )
    ...
    lines.append("=" * 72)
    hdr = (
        f"{'Inst':<10} {'PPS #int':>10} {'PPS ks':>10} "
        f"{'Pipe #int':>10} {'Pipe ks':>10} {'Match':>7}"
    )
    lines.append(hdr)
    lines.append("-" * len(hdr))

    pps_evts = {
        "PN S": "P0963720201PNS003PIEVLI0000.FTZ",
        "MOS1 S": "P0963720201M1S001MIEVLI0000.FTZ",
        "MOS2 S": "P0963720201M2S002MIEVLI0000.FTZ",
    }
    pipe_evts = {
        "PN S": "PN/4759_0963720201_EPN_S003_ImagingEvts.clean.fits",
        "MOS1 S": "M1/4759_0963720201_EMOS1_S001_ImagingEvts.clean.fits",
        "MOS2 S": "M2/4759_0963720201_EMOS2_S002_ImagingEvts.clean.fits",
    }
    ...
    for label in ["PN S", "MOS1 S", "MOS2 S"]:
        ...
        lines.append(
            f"{label:<10} {pn:>10d} {pk:>10.1f} {pipn:>10d} {pipk:>10.1f} {ok:>7}"
        )
    ...
    pps_fbk = {
        "PN": "P0963720201PNS003FBKTSR0000.FTZ",
        "MOS1": "P0963720201M1S001FBKTSR0000.FTZ",
        "MOS2": "P0963720201M2S002FBKTSR0000.FTZ",
    }
    pipe_espfilt = {
        "PN": "PN/4759_0963720201_EPN_S003_ImagingEvts.clean.espfilt/pnS003-gti.fits",
        "MOS1": "M1/4759_0963720201_EMOS1_S001_ImagingEvts.clean.espfilt/mos1S001-gti.fits",
        "MOS2": "M2/4759_0963720201_EMOS2_S002_ImagingEvts.clean.espfilt/mos2S002-gti.fits",
    }
    ...
    lines.append(
        "    Consider raising rangescale further or using the PPS FBKTSR GTIs."
    )
    lines.append("")

    # ---- 3. Event counts (scheduled) ----
    lines.append("=" * 72)
    ...
    for sf in pps_src_files:
        if "EPX" in sf.name:
            with fits.open(str(sf)) as hdul:
                ...
                ml = d["EP_DET_ML"]
                lines.append(
                    f"PPS:      {len(d)} EPIC sources "
                    f"(ML {ml.min():.1f} - {ml.max():.1f})"
                )
    ...
    if pipe_src.exists():
        with fits.open(str(pipe_src)) as hdul:
            ...
            ml = d["DET_ML"]
            lines.append(
                f"Pipeline: {len(d)} curated sources "
                f"(ML {ml.min():.1f} - {ml.max():.1f}), "
                f"band PI 1000-12000"
            )
    ...
    if att_files:
        with fits.open(str(att_files[0])) as hdul:
            ...
            lines.append(
                f"PPS attitude: {dt:.1f} ks, "
                f'RA span {(ra.max()-ra.min())*3600:.1f}", '
                f'DEC span {(dec.max()-dec.min())*3600:.1f}"'
            )
    ...
    for inst in ["PN", "MOS1", "MOS2"]:
        with fits.open(str(mt_dir / pps_fbk[inst])) as hdul:
            ...
            r = r[good]
            lines.append(
                f"PPS {inst}: median={np.median(r):.2f}, "
                f"mean={np.mean(r):.2f}, "
                f"max={r.max():.1f} ct/s, "
                f"P90={np.percentile(r,90):.1f}, "
                f"P95={np.percentile(r,95):.1f}, "
                f"P99={np.percentile(r,99):.1f}"
            )
    lines.append("")
