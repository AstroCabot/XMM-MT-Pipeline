...
def read_xy_wcs(event_file: str) -> dict[str, float]:
    ...
    if abs(abs(x_arcsec_per_phys) - abs(y_arcsec_per_phys)) > 1e-06:
        raise RuntimeError(f'X and Y event scales differ unexpectedly: {x_arcsec_per_phys} vs {y_arcsec_per_phys} arcsec/phys')
    return {'x_ref_phys': xref, 'y_ref_phys': yref, 'x_ref_world': xval, 'y_ref_world': yval, 'x_arcsec_per_phys': x_arcsec_per_phys, 'y_arcsec_per_phys': y_arcsec_per_phys, 'scale_arcsec_per_phys': 0.5 * (abs(x_arcsec_per_phys) + abs(y_arcsec_per_phys)), 'x_tlmin': xmin, 'x_tlmax': xmax, 'y_tlmin': ymin, 'y_tlmax': ymax}

def offset_to_xy(wcs: dict[str, float], dx_east_arcsec: float, dy_north_arcsec: float) -> tuple[float, float]:
...
def build_region_expr(wcs: dict[str, float], shape: str, dx: float, dy: float, r: float | None=None, rin: float | None=None, rout: float | None=None) -> tuple[str, float, float]:
...
def image_grid(wcs: dict[str, float], radius_arcsec: float, bin_arcsec: float, center_dx: float=0.0, center_dy: float=0.0, center_xy_phys: tuple[float, float] | None=None) -> dict[str, Any]:
    if radius_arcsec <= 0:
        raise RuntimeError('radius_arcsec must be > 0')
    if bin_arcsec <= 0:
        raise RuntimeError('bin_arcsec must be > 0')
    if center_xy_phys is not None:
        cx, cy = center_xy_phys
    else:
        cx, cy = offset_to_xy(wcs, center_dx, center_dy)
    scale = wcs['scale_arcsec_per_phys']
    bin_phys = round(bin_arcsec / scale)
    if bin_phys < 1:
        raise RuntimeError(f'Requested image bin ({bin_arcsec} arcsec) is smaller than one X/Y physical pixel ({scale} arcsec)')
    ...
    return {'center_dx_arcsec': float(center_dx), 'center_dy_arcsec': float(center_dy), 'center_x_phys': float(cx), 'center_y_phys': float(cy), 'x_min_phys': float(xmin), 'x_max_phys': float(xmax), 'y_min_phys': float(ymin), 'y_max_phys': float(ymax), 'bin_arcsec': float(bin_arcsec), 'bin_phys': float(bin_phys), 'nx': int(nx), 'ny': int(ny), 'radius_arcsec': float(radius_arcsec), 'scale_arcsec_per_phys': float(scale), 'x_arcsec_per_phys': float(wcs['x_arcsec_per_phys']), 'y_arcsec_per_phys': float(wcs['y_arcsec_per_phys']), 'x_ref_phys': float(wcs['x_ref_phys']), 'y_ref_phys': float(wcs['y_ref_phys'])}

def cmd_xy(args: argparse.Namespace) -> int:
    wcs = read_xy_wcs(args.event)
    for key in ['x_ref_phys', 'y_ref_phys', 'x_arcsec_per_phys', 'y_arcsec_per_phys', 'scale_arcsec_per_phys', 'x_tlmin', 'x_tlmax', 'y_tlmin', 'y_tlmax']:
        print(f'{key.upper()}={wcs[key]}')
    return 0

def cmd_region(args: argparse.Namespace) -> int:
    wcs = read_xy_wcs(args.event)
    if args.center_mode == 'median':
        with fits.open(args.event) as hdul:
            evdata = hdul['EVENTS'].data
            wcs['x_ref_phys'] = float(np.median(evdata['X']))
            wcs['y_ref_phys'] = float(np.median(evdata['Y']))
    expr, x, y = build_region_expr(wcs=wcs, shape=args.shape, dx=float(args.dx), dy=float(args.dy), r=args.r, rin=args.rin, rout=args.rout)
...
def cmd_image_grid(args: argparse.Namespace) -> int:
    ...
    if args.center_mode == 'median':
        with fits.open(args.event) as hdul:
            evdata = hdul['EVENTS'].data
            center_xy = (float(np.median(evdata['X'])), float(np.median(evdata['Y'])))
    grid = image_grid(wcs=wcs, radius_arcsec=float(args.radius_arcsec), bin_arcsec=float(args.bin_arcsec), center_dx=float(args.center_dx), center_dy=float(args.center_dy), center_xy_phys=center_xy)
    if args.out_json:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap_reg.add_argument('--dx', type=float, default=0.0, help='Offset east of comet center in arcsec')
    ap_reg.add_argument('--dy', type=float, default=0.0, help='Offset north of comet center in arcsec')
    ...
    ap_reg.add_argument('--center-mode', choices=['wcs', 'median'], default='wcs', help='Reference X/Y origin: event WCS reference pixel (default) or median event position.')
    ...
    ap_img.add_argument('--center-mode', choices=['wcs', 'median'], default='wcs', help='Image-grid centre: event WCS reference pixel (default) or median event position.')
