#!/usr/bin/env python3
"""XMM event geometry utilities.

Subcommands:
    region     -- compute a SAS filtering expression for a sky region
    image-grid -- compute the DETX/DETY grid parameters for comet-frame imaging
"""
...
def read_xy_wcs(event_file: str) -> dict[str, float]:
    ...
    if abs(abs(x_arcsec_per_phys) - abs(y_arcsec_per_phys)) > 1e-06:
        raise RuntimeError(
            f"X and Y event scales differ unexpectedly: {x_arcsec_per_phys} vs {y_arcsec_per_phys} arcsec/phys"
        )
    return {
        "x_ref_phys": xref,
        "y_ref_phys": yref,
...
def image_grid(
    ...
    center_dx: float = 0.0,
    center_dy: float = 0.0,
    center_xy_phys: tuple[float, float] | None = None,
) -> dict[str, Any]:
    if radius_arcsec <= 0:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap_reg.add_argument(
        ...
        choices=["wcs", "median"],
        default="wcs",
        help="Reference X/Y origin: event WCS reference pixel (default) or median event position.",
    )
    ...
    ap_img.add_argument(
        ...
        choices=["wcs", "median"],
        default="wcs",
        help="Image-grid centre: event WCS reference pixel (default) or median event position.",
    )
