#!/usr/bin/env python3
"""Curate an emldetect source list for use in the comet pipeline.

Filters by minimum detection likelihood, optionally excludes sources
near the comet track, and writes both a FITS and DS9 region file.
"""
...
def deduplicate_positions(
    ra_deg: np.ndarray, dec_deg: np.ndarray, tol_arcsec: float
) -> np.ndarray:
...
def write_ds9(
    ds9_path: str, ra_deg: np.ndarray, dec_deg: np.ndarray, radius_arcsec: float
) -> None:
...
def main() -> int:
    ...
    ap.add_argument(
        "--input", required=True, help="Input emldetect/edetect_chain source-list FITS"
    )
    ...
    ap.add_argument(
        "--min-det-ml", type=float, default=10.0, help="Minimum detection likelihood"
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--track", help="Track FITS for comet position as a function of time"
    )
    ap.add_argument(
        "--dedup-tol-arcsec",
        type=float,
    ...
    if tab is None or len(tab) == 0:
        cols = fits.ColDefs(
            [
                fits.Column(
                    name="SRCID", format="J", array=np.array([], dtype=np.int32)
                ),
                fits.Column(
                    name="RA",
                    format="D",
                ...
                fits.Column(
                    name="DET_ML", format="E", array=np.array([], dtype=np.float32)
                ),
                fits.Column(
                    name="EXTENT",
                    format="E",
    ...
    detml = (
        np.asarray(rows[detml_col], dtype=float)
        if detml_col
        else np.full(len(rows), np.nan)
    )
    extent = (
        np.asarray(rows[extent_col], dtype=float)
        if extent_col
        else np.zeros(len(rows), dtype=float)
    )
    srcid = (
        np.asarray(rows[srcid_col], dtype=int)
        if srcid_col
        else np.arange(1, len(rows) + 1, dtype=int)
    )
    ...
    cols = fits.ColDefs(
        [
            fits.Column(
                name="SRCID", format="J", array=np.asarray(srcid, dtype=np.int32)
            ),
            fits.Column(
                name="RA",
                format="D",
                unit="deg",
                array=np.asarray(ra, dtype=np.float64),
            ),
            fits.Column(
                name="DEC",
                format="D",
                unit="deg",
                array=np.asarray(dec, dtype=np.float64),
            ),
            fits.Column(
                name="DET_ML", format="E", array=np.asarray(detml, dtype=np.float32)
            ),
            fits.Column(
                name="EXTENT",
                format="E",
    ...
    out_hdu.header["HISTORY"] = (
        f"Deduplicated positions within {args.dedup_tol_arcsec:.3f} arcsec"
    )
