#!/usr/bin/env python3
"""Curate an emldetect / edetect_chain source list for comet work.

This keeps the summary rows (ID_INST==0 and ID_BAND==0 when present), applies a
minimum detection-likelihood cut, optionally removes sources close to the comet
track, de-duplicates positions, and writes a compact SRCLIST table.

The output is meant for downstream contamination handling and visual checks,
not for direct scientific catalog work.
"""
...
def deduplicate_positions(ra_deg: np.ndarray, dec_deg: np.ndarray, tol_arcsec: float) -> np.ndarray:
    """Return boolean mask that keeps one source per position cluster.

    A simple O(N^2) pass is fine here because XMM source lists are small.
    The brightest/highest-DET_ML source should already have been sorted first.
    """
...
def main() -> int:
    ...
    ap.add_argument(
        "--exclude-track-radius-arcsec",
        type=float,
        default=0.0,
        help="Exclude detections within this distance of the comet track",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=1.0,
        help="Deduplicate sources closer than this angular distance",
    )
    ap.add_argument(
        "--ds9-radius-arcsec",
        type=float,
        default=20.0,
        help="Circle radius for optional DS9 output",
    )
    args = ap.parse_args()
    ...
    if tab is None or len(tab) == 0:
        cols = fits.ColDefs(
            [
                fits.Column(name="SRCID", format="J", array=np.array([], dtype=np.int32)),
                fits.Column(name="RA", format="D", unit="deg", array=np.array([], dtype=np.float64)),
                fits.Column(name="DEC", format="D", unit="deg", array=np.array([], dtype=np.float64)),
                fits.Column(name="DET_ML", format="E", array=np.array([], dtype=np.float32)),
                fits.Column(name="EXTENT", format="E", unit="arcsec", array=np.array([], dtype=np.float32)),
            ]
        )
    ...
    # Sort brightest first so dedup keeps the most significant source.
    ...
    cols = fits.ColDefs(
        [
            fits.Column(name="SRCID", format="J", array=np.asarray(srcid, dtype=np.int32)),
            fits.Column(name="RA", format="D", unit="deg", array=np.asarray(ra, dtype=np.float64)),
            fits.Column(name="DEC", format="D", unit="deg", array=np.asarray(dec, dtype=np.float64)),
            fits.Column(name="DET_ML", format="E", array=np.asarray(detml, dtype=np.float32)),
            fits.Column(name="EXTENT", format="E", unit="arcsec", array=np.asarray(extent, dtype=np.float32)),
        ]
    )
    out_hdu = fits.BinTableHDU.from_columns(cols, name="SRCLIST")
    ...
    if args.track and args.exclude_track_radius_arcsec > 0:
        out_hdu.header["HISTORY"] = (
            f"Excluded sources within {args.exclude_track_radius_arcsec:.3f} arcsec of comet track"
        )
