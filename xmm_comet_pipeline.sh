...
###############################################################################
# Self-contained XMM-Newton comet pipeline
#
# Stages:
#   init repro clean track detect comet contam image lcurve spectrum merge qc
#   science = clean track detect comet contam image lcurve spectrum merge
#   all     = init repro clean track detect comet contam image lcurve spectrum merge qc
#
# The main design changes in this refactor are:
#   * multiple raw event lists per instrument are handled explicitly,
#   * per-exposure cleaned files are merged per instrument before track/comet work,
#   * still-sky detection uses emosaic_prep + edetect_stack,
#   * the moved attitude is built by cloning the whole ODF tree and replacing the
#     original spacecraft attitude history file (*SC*ATS.FIT*),
#   * contamination GTIs use the UTC->TIMESYS-aware builder,
...
setup_sas_env() {
  ...
  # Re-derive paths through current $WORKDIR (may be a short symlink that
  # differs from the PID that ran stage_init).
  export SAS_CCF="$WORKDIR/init/ccf.cif"
  local sumsas
...
load_attitude() {
  ...
  # Re-derive paths through current $WORKDIR
...
EOF
  ...
  # then rewrite the SUM.SAS PATH to point here. This avoids the SAS Perl
  # wrapper glob matching encrypted archives alongside the real files.
  local odf_file
...
# Write comprehensive exposure manifest for QC
...
PY
  ...
  if [[ "$FORCE" == "1" || ! -s "$WORKDIR/detect/stack_srclist.fits" ]]; then
    edetect_stack       eventsets="@stack_eventsets.txt"       attitudesets="$tmpdet/detect/atthk.dat"       summarysets="$tmpdet/detect/SUM.SAS"       pimin="$SRCDET_PI_MIN"       pimax="$SRCDET_PI_MAX"       mlmin="$DETECT_MIN_ML"       srclistset="srclist.fits"       informational=all       compress=false       prefix="stack_"       emos_mosaicedset="mosaic"       > "$LOGDIR/detect_edetect_stack.log" 2>&1
  ...
  local low high
  low=$(printf '%05d' "$SRCDET_PI_MIN")
  high=$(printf '%05d' "$SRCDET_PI_MAX")
  for cand in     "$WORKDIR/detect/stack_mosaic_EPIC_${low}_${high}.fits"     "$WORKDIR/detect/mosaic_EPIC_${low}_${high}.fits"
  do
    if [[ -f "$cand" ]]; then
  ...
  while read -r qc_line; do
    ...
    python3 "$HELPER_DIR/make_stillsky_mosaic.py"       --events "${qc_events[@]}"       --pimin "$qc_pimin"       --pimax "$qc_pimax"       --track "$WORKDIR/track/comet_track.fits"       --srclist "$WORKDIR/detect/field_sources_all.fits"       --bin-arcsec "$DETECT_QC_BIN_ARCSEC"       --pad-arcmin "$DETECT_QC_PAD_ARCMIN"       --label "$qc_label"       --out-fits "$WORKDIR/detect/EPIC_${qc_label}_mosaic.fits"       --out-json "$WORKDIR/detect/EPIC_${qc_label}_mosaic.json"
...
try:
    ...
    # Fall back to basename search within the ODF tree.
...
stage_contam() {
  ...
  if bool_yes "$SCIENCE_APPLY_TRAIL_MASK"; then
    python3 "$HELPER_DIR/make_trail_mask.py"       --grid-json "$WORKDIR/images/grid.json"       --track "$WORKDIR/track/comet_track.fits"       --srclist "$contam_srclist"       --mask-radius-arcsec "$IMAGE_MASK_RADIUS_ARCSEC"       --out-mask "$WORKDIR/contam/EPIC_trail_mask.fits"       --out-json "$WORKDIR/contam/EPIC_trail_mask.json"
    needs_mask=1
  else
    needs_mask=0
  fi
  ...
  for inst in $(selected_insts); do
    ...
    if [[ $needs_mask -eq 1 ]]; then
      python3 "$HELPER_DIR/apply_spatial_trail_mask.py"         --event "$in_evt"         --mask "$WORKDIR/contam/EPIC_trail_mask.fits"         --grid-json "$WORKDIR/images/grid.json"         --output "$out_evt"         --report-json "$WORKDIR/contam/${inst}_science_base.json"
    else
      cp -f "$in_evt" "$out_evt"
    fi
  done
